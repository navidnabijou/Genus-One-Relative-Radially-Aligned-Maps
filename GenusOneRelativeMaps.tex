...
\subsection{Wonderful trick to compute reduced rubber integrals}

Our method to compute the rubber VZ integrals which appear in the recursion:

First, if the degree is zero, then it's Pixton's formula minus the locus with a single rational tail containing all the markings. So focus on positive degree case.
...
\begin{equation*} \Lcal_k = x_k^\st\pi^\st \Lcal_k = x_k^\st \OO_\Ccal(\alpha_k x_k) \otimes x_k^\st f^\st\OO(H) = x_k^\st \omega_{\Ccal/S}^{\otimes \alpha_k} \otimes \ev_k^\st \OO(H) \end{equation*}
and taking Chern classes gives:
\begin{equation*} \cchern_1(\Lcal_k) = \alpha_k \psi_k + \ev_k^\st H.\end{equation*}
This gives the construction of $(\Lcal_k,s_k)$ on $\MLog_{1,\alpha}(\PP^N|H)$; the construction on $\VZ_{1,\alpha}(\PP^N|H)$ is given by pull-back. Notice in particular that $\psi_k$ should be interpreted as a \emph{collapsed psi class} on $\VZ_{1,\alpha}(\PP^N|H)$, i.e. a psi class coming from the stabilised curve of the collapsed map. All of the psi classes we deal with will be collapsed psi classes.

\begin{remark} This construction gives the natural logarithmic analogue of Gathmann's line bundle \cite[Construction 2.1]{Ga}. One of the benefits of the logarithmic approach is to make the computation of multiplicities entirely combinatorial; see \S \ref{} below.\end{remark}
