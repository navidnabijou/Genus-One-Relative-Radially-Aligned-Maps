...
\subsubsection{Restricting the possible combinatorial types} Recall that a boundary divisor $\Dcal \subseteq \Dcal_{1,\alpha,k}(\PP^N|H,d)$ is said to be of \textbf{type $C^0$} if the core is assigned zero degree by the combinatorial type. In this case, the combinatorial type is not as tightly constrained as previously (it need not be a bipartite graph with a single interior vertex). Nevertheless, we have the following lemma:
\begin{lemma} 
Let $\Dcal \subseteq \Dcal_{1,\alpha,k}(\PP^N|H,d)$ be a locus of type $C^0$ and let $\Delta$ be the corresponding combinatorial type. Then for every nonzero vertex $\diamond$ of the expanded target, the fibre of the tropical map over $\diamond$ contains exactly one stable vertex (together with a number of trivial bubbles), and at least one vertex which lies on the radius (which may or may not be stable).
\end{lemma}
\begin{proof}
By stability we always need at least one stable vertex lying over each  vertex of the target. If there was more than one, then (since both the insertions and the factorisation condition are pulled back from the moduli space of maps without expansions) it can be seen, as in the proof of Lemma \ref{lem:combs}, that the contribution vanishes.
...
\subsubsection{Constructing $\widetilde\Dcal$ from $\Ecal$}
...
\subsubsection{Factorisation: obtaining $\Dcal$ from $\widetilde\Dcal$} Assume $m\geq 2$. Over $\mathcal P$ we have a tautological sequence:
