...
\begin{equation}\label{type A splitting axiom}\Dcal=\Mpunct{0}{\alpha^{(0)}\cup (-m_1,\ldots,-m_r)}{\PP^N|H}{d_0}\times_{H^r}\left(\VZ_{1,\alpha^{(1)}\cup(m_1)}(\PP^N|H,d_1)\times\prod_{i=2}^r\Mlog{0}{\alpha^{(i)}\cup(m_i)}{\PP^N|H}{d_i}\right) \end{equation}
i.e. that the map $\Dcal\to\Dcal_\Mcal$ is given by blowing up one of the factors of the fibre product. Note that the map $\Dcal\to\Dcal_\Mcal$ is an isomorphism away from the blown-up locus, which lives inside the locus where the circuit is contracted. 
