...
\subsection{Singularities and moduli of attachments}\label{S:ellsing} Isolated Gorenstein singularities of genus $1$ were classified by Smyth in \cite{SMY1}: for every fixed number of branches $m$, there is only one isomorphism type of curve germs with such characteristics. For $m=1,2$ we find the cusp ($A_2$) and tacnode ($A_3$) respectively; for $m\geq 3$ we find the union of $m$ general lines through the origin of $\Aaff^{m-1}$ - Smyth calls them \emph{elliptic $m$-fold points}.
