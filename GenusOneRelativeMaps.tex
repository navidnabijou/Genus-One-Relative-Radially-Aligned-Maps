...
\end{tikzpicture}
\end{center}
Geometrically, there are four codimension--$1$ logarithmic strata of $\VZ$ intersecting our chosen neighbourhood, corresponding to the rays in the above picture. Two of these -- those corresponding to $\{ e_1=e_2=0 \}$ and $\{ e_3=0 \}$ -- are the proper transforms of codimension--$1$ strata in $\Mcal$. Here some of the edge lengths are set to zero, and the corresponding logarithmic strata consist of  stable maps where the relevant nodes are smoothed. Notice that although the curve has three nodes, there are only two such strata: the nodes $q_1$ and $q_2$ cannot be smoothed independently because of the relation $e_1=2e_2$.

The remaining two codimension--$1$ strata correspond to where certain of the vertex distances become equal. In these strata, none of the nodes are smoothed. From the construction of the subdvision, we see that they both map onto the codimension--$2$ stratum of $\Mcal$ in which all of the nodes persist; this coheres with the fact that they should be thought of as exceptional loci of the blow-up. The extra dimension of moduli comes from the choice of alignment.

There are three codimension--$2$ strata corresponding to different strict orderings of the vertex distances. Note that the strata corresponding to $\{e_1=e_2=0\}$ and $\{e_3=0\}$, which intersected in $\Mcal$, no longer intersect in $\VZ$. The picture is something like:
\begin{center}
