...
\title{Absolute, relative and rubber Gromov--Witten theory of hypersurfaces in genus one}
...
is a log modification, $\widetilde{\Dcal}$ is either exceptional or the proper transform of a log divisor. These cases correspond, respectively, to when the circuit is assigned zero degree or nonzero degree by the combinatorial type. To begin with we will focus on the latter case. Suppose therefore that $\widetilde{\Dcal}$ is the proper transform of a log divisor:
...
\subsubsection{Combinatorial types} Recall that a boundary divisor $\Dcal \subseteq \Dcal_{1,\alpha,k}(\PP^N|H,d)$ is said to be of \textbf{type $C^0$} if the core is assigned zero degree by the combinatorial type. In this case, the combinatorial type is not as tightly constrained as previously (it need not be a bipartite graph with a single interior vertex). Nevertheless, we have the following lemmas:
\begin{lemma} 
Let $\Dcal \subseteq \Dcal_{1,\alpha,k}(\PP^N|H,d)$ be a locus of type $C^0$ and let $\Delta$ be the corresponding combinatorial type. Then for every nonzero vertex $\diamond$ of the expanded target, the fibre of the tropical map over $\diamond$ contains exactly one stable vertex (together with a number of trivial bubbles), and at least one vertex which lies on the circle (which may or may not be stable).
\end{lemma}
\begin{proof}
...
Furthermore, for each vertex of the target there must always be a vertex in the preimage which lies on the radius, since otherwise there would be at least two independent tropical parameters, and so $\Dcal$ would not be a divisor.\end{proof}
...
where $E$ is the set of edges of the (stabilised) dual graph, and the fibre product is taken over the appropriate evaluation maps. Note that only the factor corresponding to the core vertex has non-zero genus, and that it must have zero degree.
...
\begin{comment}
In this case, the core has (horizontal) degree $0$ and is mapped into $H$. We start by describing the relevant combinatorial types of rays in $\widetilde{\VZ}_{1,\alpha}(\PP^N|H,d)$.
\begin{lemma} In what follows it is understood that we insert trivial bubbles whenever the curve or target need to be subdivided in order for the image and preimage of every polyhedral subcomplex to be a union of polyhedral subcomplexes.
 \begin{enumerate}
  \item The core is represented by a single vertex of genus one and degree zero, tropically mapped to $\RR_{>0}$.
  \item The vertices adjacent to the core with maximal expansion factor are tropically mapped to $0$.
  \item For every point of the target different from $0$, the fiber of the tropical map contains at most one stable vertex, which either is the core or lies on the circle.
  \item $x_k$ is supported by a vertex tropically mapped to $\RR_{>0}$.
 \end{enumerate}
\end{lemma}
\begin{proof}
 By assumption, the core is mapped to $\RR_>0$, hence the distance of its image from $0$ is one parameter $\ell$; since we are describing a ray, all other parameters have to be related to $\ell$, and the only way it can be so is via the alignment. If the fiber over $c>0$ contains two stable vertices, because both the insertions and the factorisation condition are pulled back from the moduli space without expansions, it can be seen as in Lemma \ref{lem:combs} that the collapsing morphism $\Dcal\to\Fcal$ has positive-dimensional fibers (disconnected rubber), so the corresponding combinatorial type does not contribute to the invariants. We conclude that every stable vertex over $\RR_{>0}$ different from the core has to lie on the circle.
\end{proof}
\begin{center}
\begin{figure}
 \begin{tikzpicture}[ipe import]
...
See Figure \ref{fig:ray_example}. We observe that the corresponding stratum $\Ecal$ of $\ol\Mcal_{1,\alpha}(\PP^N|H,d)$ has logarithmic codimension equal to the number of non-core vertices mapping to $\RR_{>0}$: indeed, the combinatorial type of $\Ecal$ can be read off by forgetting the circle, and the logarithmic codimension in $\ol\Mcal_{1,\alpha}(\PP^N|H,d)$ is accounted for by the number of subdivisions of the target. The discrepancy in dimension is to be expected, as the loci of type $C^0$ are contained in the exceptional locus of the blow-up.

The geometry of $\widetilde\Dcal\to\Ecal$ is encoded by the combinatorics of the alignment. The reader should bear the following toric construction in mind: suppose we subdivide a fan, and we want to describe the relation between the varieties corresponding respectively to a cone $\rho$ of the subdivision, and the original cone $\sigma$ containing it. The fan of $X_\tau$ is obtained by projecting the star of $\tau$ to the quotient $N_\tau=N_{\RR}/\langle\tau\rangle_{\RR}$ of the ambient vector space by the linear span of the cone\footnote{We find it useful to encode this within the formalism of extended tropicalisations \cite{?}.}. Notice that the dimension of the vector spaces $N_\rho$ and $N_\sigma$ (or the corresponding toric varieties) is off by the difference $c$ between the dimension of the cones. Furthermore, we may need to subdivide the fan of $X_\sigma$ in order for cones of $\Sigma_\rho$ to be mapped surjectively to cones of $\Sigma_\sigma$. The geometric interpretation of this is that $X_\tau$ is obtained as an appropriate compactification of a toric bundle of dimension $c$ over $X_\sigma$; observe that the fiber dimension may not be constant (but it can be made so by blowing up $X_{\sigma}$, see \cite[Lemma 4.1]{AK}). It is a fundamental insight due to Tevelev \cite{Tevelev} that a good compactification of the toric bundle is suggested by the tropicalisation of the locus we are interested in: here, ``good'' means that the intersection of the interesting locus with the boundary is dimensionally transverse. In the case at hand, the interesting locus is that of maps satisfying the factorisation property (i.e. smoothable maps), and the tropicalisation has been worked out in \cite{RSPW2} to consist of well-spaced maps.

We proceed to describe $\Dcal$ step-by-step, building on $\Ecal$.


\subsubsection{The fiber product} We start from the finite splitting morphism
\begin{multline}\label{eq:product}
 \rho\colon \Ecal \to \left(\prod_{i=1}^k \ol\Mcal_{0,\alpha^{(i)}\cup(m)}(\PP^N|H,d_i) \times \prod_{i=k+1}^s \ol\Mcal_{0,\alpha^{(i)}\cup(m_i)}(\PP^N|H,d_i)\right)\times_{H^s} \\ \prod_{j=k+1}^{r}\ol\Mcal_{0,\alpha^{(j)}\cup(-m_i)_{i \in I_j\subseteq\{k+1,\ldots,s\}}\cup(\mu_j)}^{\leftrightarrow}(P|H_0+H_\infty,d_j)\times_{H^{r-k}} \\
...
\end{multline}
The degree of $\rho$ can be computed as in Lemma \ref{lem:saturation} above. From the logarithmic point of view, $\rho$ is a saturation map; generically it should be thought of as follows. On the fiber product, the tangent vectors of the components on level $0$ (corresponding to the factors in the first line above) are identified with the target tangent line up to finite powers; the saturation is taking roots of these isomorphisms (it is interesting to notice that it fails precisely on Gathmann's boundary comb loci).
\end{comment}

\subsubsection{From $\Ecal$ to $\widetilde{\Dcal}$} The reader should bear the following construction from toric geometry in mind: suppose we subdivide a fan, and we want to describe the relation between the varieties corresponding respectively to a cone $\rho$ of the subdivision, and the original cone $\sigma$ containing it. The fan of $X_\tau$ is obtained by projecting the star of $\tau$ to the quotient $N_\tau=N_{\RR}/\langle\tau\rangle_{\RR}$ of the ambient vector space by the linear span of the cone\footnote{We find it useful to encode this within the formalism of extended tropicalisations \cite{?}.}. Notice that the dimension of the vector spaces $N_\rho$ and $N_\sigma$ (or the corresponding toric varieties) is off by the difference $c$ between the dimension of the cones. The image of $\Sigma_\rho$ provides a subdivision of the fan of $X_\sigma$. The geometric content is that $X_\tau$ is obtained as an appropriate compactification of a toric bundle of dimension $c$ over $X_\sigma$.
...
We modify the boundary of $\Ecal$\footnote{With the log structure obtained by pulling back from Kim's space and modding out by $\mathbb N^{\codim (\sigma)}$, i.e. considering the edges of $E$ to have \emph{infinite length}.} as follows:
\begin{enumerate}
 \item radially aligning the core, and imposing factorisation, thus effectively replacing
 \[\ol\Mcal^{\leftrightarrow}_{1,\mu}(P|H_0+H_\infty)\quad\text{by}\quad\VZ^{\leftrightarrow}_{1,\mu}(P|H_0+H_\infty)\]
 in the product above, with the consequence of introducing a reduced rubber;
 \item requiring that any degeneration of $V_{0,\delta}$ (resp. $\forall v\in V_{>0,\delta}$) remains centrally aligned with respect to the core;\footnote{Notice that this requires us to blow up some diagonal subloci of the product above, see Figure \ref{fig:fwise_align}} this has the effect that on the whole of $\widetilde E$ - including the boundary -, for each $v\in \{V_{0,\delta}\}\cup V_{>0,\delta}$, it is always possible to tell the components of the curve that are at distance $\delta$ from the core along the path denominated $v$, and, if there is more than one for the same $v$, the lengths of these paths are identified, so as to define line bundles $\OO(\delta_v), \forall v\in \{V_{0,\delta}\}\cup V_{>0,\delta}$.
\end{enumerate}
...
Notice that $\mathcal P$ is well-defined on the whole of $\widetilde \Ecal$, while $\widetilde \Dcal\to \mathcal P$ is in fact a modification oof the boundary, as dictated by the alignment procedure. In the next section, we extrapolate what we need from this combinatorial setup in order to describe $\Dcal$ explicitly.
\begin{comment}
...
\subsubsection{From $\widetilde\Dcal$ to $\Dcal$}
