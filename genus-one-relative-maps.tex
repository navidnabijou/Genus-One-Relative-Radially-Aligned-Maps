...
We now define the \emph{centrally alined relative space} to be the closure of the nice locus:
...
\begin{equation*} \langle \tau_{k_1}(\gamma_1), \ldots, \tau_{k_n}(\gamma_n) \rangle^{\PP^N|H, \operatorname{red}}_{0,\alpha,d} := \int_{[\VZc{1}{\alpha}{\PP^N|H}{d}]} \prod_{i=1}^n \ev_i^*(\gamma_i) \psi_i^{k_i} \end{equation*}
...
\begin{remark} The monoid $Q^{\operatorname{GS}}$ above is the minimal monoid for the moduli space of log stable maps, as constructed by Gross--Siebert. Its definition relies on a choice of \emph{type}, which in our setting is uniquely determined except when the source curve $C$ contains a wheel of $\PP^1$s. \end{remark}

\begin{proof}[Proof of Proposition \ref{closed pts of relative space}]
First suppose that Gathmann's condition is satisfied and that we have a factorisation:
\bcd
Q^{\operatorname{log}} \ar[r]\ar[d] & Q^{\operatorname{GS}} \ar[d] \\
Q^{\operatorname{ctr}}\ar[r] & \N
\ecd
We will construct a smoothing of the centrally aligned map, i.e. a $1$-parameter family whose central fibre is the map we started with and whose general fibre is in the nice locus.

The construction of the source curve $\Ccal \to \spec \mathbbm{k} \llbracket t \rrbracket$ is given in the usual way by starting with the circuit $E$ and performing a weighted blow-up to reconstruct the rest of the curve.\basenote{What do we do if $E$ is not smooth? We need to smooth $E$ itself, and we also need some node multiplicities at the nodes internal to $E$.} The weights are determined by the map $Q^{\operatorname{log}} \to \N$; this map is local since the map $Q^{\operatorname{log}} \to Q^{\operatorname{GS}}$ is local\basenote{Is this always true? Isn't this only true if there exists a log lifting? But maybe it's always true for Deligne--Faltings pairs.} and the map $Q^{\operatorname{GS}} \to \N$ is local by assumption. Thus each generator $e_q \in Q^{\operatorname{log}}$ maps to a positive integer $r_q$, and this is the weight we choose. This means that if $C_1$ and $C_2$ are the two branches of $C$ around $q$, then these are $\QQ$-Cartier divisors in the total space $\Ccal$, with:
\begin{equation*} C_1 \cdot C_2 = 1/r_q \end{equation*}
We now define the map $f \colon \Ccal \to \PP^N$ extending the map $f \colon C \to \PP^N$. We assume that $H=\{ z_0=0\}$ is a co-ordinate hyperplane. We have sections $u_0,\ldots,u_N$ of a line bundle $L$ on $C$ and we need to extend these to sections $\tilde{u}_0,\ldots,\tilde{u}_N$ of a line bundle $\Lcal$ on $\Ccal$.

Let us write $C = C_1 \cup \ldots \cup C_m$ with $C_1,\ldots,C_r$ internal and the rest external. We define the line bundle $\Lcal$ as
\begin{equation*} \Lcal = \OO_{\Ccal}\left(\sum_{i=1}^r \gamma_i C_i + \sum_{i=1}^n \alpha_i \tilde{x}_i \right) \end{equation*}
where each $\gamma_i$ is a positive integer obtained as the image under $Q^{\operatorname{GS}} \to \N$ of the generator corresponding to $C_i$.

This comes with a natural section (or rather, a $\CC^*$-equivalence class of sections) $\tilde{u}_0$ given by the divisor in brackets, and this can be chosen in a unique way so that it restricts to $u_0$ on the central fibre of $\Ccal$.

For each external component $C_j$, we have
...
But this is true, because in $Q^{\operatorname{GS}}$ the generator corresponding to the internal component $C_i$ is identified with $m_i$ times the generator corresponding to $q_i$; pushing forward this relation to $\N$ gives the equation above.
 
\begin{comment} 
Now let us consider an internal component $C_j$. We have
\begin{align*} \Lcal|_{C_j} & = \OO_{C_j} \left( \sum_{x_i \in C_j} \alpha_i x_i + \sum_{q_i \in C_j} (\gamma_i/r_i) q_i \right) \otimes \OO_{\mathcal{C}}\left(- \gamma_j \sum_{i \neq j} C_i \right) \\
& = \OO_{C_j}\left( \sum_{x_i \in C_j} \alpha_i x_i + \sum_{q_i \in C_j} \left(\frac{\gamma_i - \gamma_j}{r_i}\right) q_i - \sum_{\tilde{q}_i \in C_j} (\gamma_j/r_i) \tilde{q}_i \right)\end{align*}
...
It remains to verify the Gathmann-like condition for internal components. Let $Z \subseteq C$ be a connected component of $f^{-1}(H)$, and suppose wlog that $Z = C_1 \cup \ldots \cup C_k$ for some of the internal components $C_1,\ldots,C_k$. If $Z$ has genus zero then the condition is purely numerical, and we find from the above description that
\begin{equation*} \deg \Lcal|_Z = \sum_{x_i \in Z} \alpha_i x_i - \sum_{\tilde{q}_i \in Z} m_i \end{equation*}
where the sum is over external nodes $\tilde{q}_i$, i.e. those connecting to the rest of the curve. By the Gathmann condition, this is exactly $\deg f|_Z^* \OO_{\PP^N}(H)$, which is what we wanted.

On the other hand suppose that $Z$ is genus one. Suppose first off that it has a smooth genus one component, say $C_1$.
\end{comment}

Let $Z$ be a connected component of $f^{-1}(H)$. Suppose wlog that $Z= C_1 \cup \ldots \cup C_k$. Then because $Z$ does not intersect $C_{k+1},\ldots,C_r$ we have
\begin{align*} \Lcal|_Z & = \OO_Z \left( \sum_{x_i \in Z} \alpha_i x_i + \sum_{i=1}^k \gamma_i C_i \right) \\
& = \OO_Z \left( \sum_{x_i \in Z} \alpha_i x_i - \sum_{q_i \in Z} (\gamma_i/r_i) q_i \right) \\
& = \OO_Z \left( \sum_{x_i \in Z} \alpha_i x_i - \sum_{q_i \in Z} m_i q_i \right) \end{align*}
where the sum is over external components of $C$ which are adjacent to $Z$ via $q_i$. By the Gathmann-like condition, this is exactly $f^* \OO(Y)|_Z$.

We have thus shown that when we restrict $\Lcal$ to either an external component or an internal component (where here we mean a connected component of $f^{-1}(Y)$) we get the correct line bundle. Assuming that there is no wheel of $\PP^1$s, or more generally that any wheel of $\PP^1$s which does appear appears inside a connected component, this shows that $\Lcal_{\Ccal_0} = L$, as claimed.\basenote{What about in general?}
