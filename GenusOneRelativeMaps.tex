...
The base for the induction is the case of a single relative marking $x_1$ with full tangency $\alpha_1=d$. Write the insertion as $\gamma=\tau_{b_1}(\gamma_1)\tau(\gamma)_{\geq 2}$. We shall add a non-relative marking $x_0$ and, starting from the equality $c_1(L_{0,\rm{not\ bot}})\cdot[\VZ_{1,\alpha}(P|H_0+H_\infty)^\sim]=\ldots$ (the boundary classes described above), intersect it with $\bar\gamma=\fgt_{x_1}^*\left(\psi_0^{b_1}\ev_0^*(\gamma_1)\tau(\gamma)_{\geq 2}\right)$. \begin{comment}{$\bar\gamma=\fgt_{x_1}^*\left(\psi_0^{b_1}\ev_0^*(\gamma_1)\tau(\gamma)_{\geq 2}\right)$\end{comment}
