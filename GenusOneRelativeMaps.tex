...
\subsection{Fictitious and true markings} The recursion procedure is rather delicate. Roughly speaking, we will induct on the degree (meaning $Y\cdot\beta$), number of marked points and total tangency. To make sense of these last two in the relative setting, we introduce the notion of \textbf{fictitious markings}. Consider a moduli space $\VZ_{1,\alpha}(X|Y,\beta)$ of reduced relative stable maps and a corresponding integrand $\gamma$. We let $F \subseteq \{1,\ldots,m\}$ be the maximal subset of marked points such that:
\begin{enumerate}
...
The left-hand side is a forgetful relative invariant of degree $d$ and $\leq n+1$ true markings, and so has already been computed. For the right-hand, let us begin with loci of type $A$. The genus one contributions from each loci have $(d^\prime,n^\prime) < (d,n)$ except in the following case
\begin{center}
\begin{tikzpicture}[scale=1]
%edge
\draw (0,0) to (2,0);
...
\begin{equation*} d\cdot \tilde\gamma \cap [\VZ_{1,(d,\underbrace{0,\ldots,0}_{n-1})}(X|Y,\beta)] \end{equation*}
to the right-hand side of \eqref{step 2 recursion}. The type $B$ and $C^0$ loci are recursively determined as in Step~1, and similarly the type $C^+$ loci are recursively determined except for the locus where the entire curve is mapped into the divisor. On this locus we may apply $\fgt_{x_0}$ and identify the contribution with
...
\begin{equation}\label{step 2 recursion 2} I = (-\tilde\gamma/d) \cap [\VZ_{1,(d,0,\ldots,0)}(X|Y,\beta)] + \text{recursively known terms} \end{equation}
...
where $x_1$ now has tangency $d-1$ and we have introduced a new marking $y$ with tangency $1$. We obtain a new insertion, also denoted $\tilde\gamma$, by introducing $\fgt_y^\st$ everywhere. Recursing at $x_1$ we obtain:
\begin{equation*} \left( (d-1)\psi_1 + \ev_1^\st Y \right)(-\tilde\gamma/d) \cap [\VZ_{1,(d-1,1,0,\ldots,0)}(X|Y,\beta)] = (-\tilde\gamma/d) \cap [\Dcal(1)].\end{equation*}
The left-hand side is a forgetful relative invariant of degree $d$ and $n+1$ markings ($y$ being the ``forgetful'' marking), hence is recursively known. On the right-hand side, the type $A$ loci are recursively known except possibly in the following special cases (with some stable distribution of the remaining markings):
\begin{center}
...
\draw (2,-1.5) node[below]{\small{Case 1}};
\end{tikzpicture}
\end{minipage}
\begin{minipage}{0.4\textwidth}
\begin{tikzpicture}[scale=1]
...
\draw (2,-1.5) node[below]{\small{Case 2}};
\end{tikzpicture}
\end{minipage}
\end{center}
In Case 1 the contribution from $\sqC_1$ is a forgetful relative invariant with $\leq n+1$ markings, hence is recursively known. In Case 2, we first note that there cannot be any more markings on $\sqC_0$ (since otherwise we could apply $\fgt_y$ to conclude that the contribution vanishes). Thus we obtain a single locus, which contributes precisely
\begin{equation*} d\cdot(-\tilde\gamma/d) \cap [\VZ_{1,(d,0,\ldots,0)}(X|Y,\beta)] = -\tilde\gamma\cap[\VZ_{1,(d,0,\ldots,0)}(X|Y,\beta)]\end{equation*}
which is the first term on the right-hand side of \eqref{step 2 recursion 2}. As usual the type $B$ and $C^0$ contributions are known recursively, and the only type $C^+$ contribution not known recursively occurs when the entire curve is mapped into the divisor, in which case we obtain:
\begin{equation*} d^2\cdot(-\gamma/d) \cap [\VZ_{1,n}(X|Y,\beta)] = -d\cdot\gamma \cap [\VZ_{1,n}(X|Y,\beta)].\end{equation*}
Rearranging in \eqref{step 2 recursion 2} we end up with:

WHOOPS! WE'VE CANCELED OUT! NEED TO TRY HARDER.
