...
\begin{lemma} Let $\Dcal \subseteq \Dcal(k)$ be an irreducible component which contributes nontrivially to the recursion, and let $\Delta$ be the corresponding combinatorial type. Suppose that $\Delta$ assigns positive degree to the circuit. Then $\Delta$ takes one of the following forms:
\begin{figure}[h]
    \centering
...
\subsubsection{Type $A$} Let $\Delta$ be a combinatorial type of type $A$ and let $\Dcal\subseteq\VZ_{1,\alpha}(\PP^N|H,d)$ be the corresponding logarithmic divisor. The corresponding locus $\Dcal^{\Kim} \subseteq \ol\Mcal^{\Kim}_{1,\alpha}(\PP^N|H,d)$ admits a finite splitting morphism to the fibre product
...
and is calculated in the usual way, by examining the base order of the corresponding family of targets \cite[\S 5]{ACGSDecomposition}. 
...
be the piecewise-linear function constructed in \S \ref{Section Gathmann line bundle}. It follows from the tropical description that $\lambda_\Dcal$ is equal to the index of the map $\tau \to \RR_{\geq 0}$ obtained by restricting $\varphi$. The cone $\tau$ has as an integral generator $f=m_i e_i/\lcm(m_1,\ldots,m_r)$. Given $v \in \tau$ the target edge length $\varphi(v)$ is given by
\begin{equation*} l=m_i e_i = \lcm(m_1,\ldots,m_r) f \end{equation*}
which shows that the index $\lambda_\Dcal = \lcm(m_1,\ldots,m_r)$.\end{proof}

Lemma \ref{lemma vanishing order type A} together with the degree calculation \eqref{degree of gluing} determines the numerics of integrals over $\lambda_\Dcal [\Dcal]$.
