...
On the target side, it would be natural to generalise the method of degenerating tangency conditions to higher rank logarithmic structures. When the target is a toric pair the problem is tied in with the enumerative geometry of well-spaced tropical curves~\cite{LR18,RSPW2,Speyer}. %We will return to these questions in future work. 
...
\begin{figure}
 ...
 \tikzset{cross/.style={cross out, draw=black, fill=none, minimum size=2*(#1-\pgflinewidth), inner sep=0pt, outer sep=0pt}, cross/.default={3pt}}
  ...
  \draw (1,0) node{$\vdots$};
  \draw[->] (4,0) --node[below]{\color{blue} $d$} (6,0);
  \draw[blue,->] (0,-2)--(6,-2);
...
Step 2, computing the reduced absolute invariant of $Y$ of degree $d$ and $n$ markings, is complete.
