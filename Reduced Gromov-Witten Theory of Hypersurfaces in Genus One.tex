...
\subsubsection{Description of the possible combinatorial types} Recall that a boundary divisor $\Dcal \subseteq \Dcal_{1,\alpha,k}(\PP^N|H,d)$ is said to be of type IIIb if the core is assigned degree zero by the combinatorial type (this is the only case which we did not treat in Lemma \ref{lem:combs}). In this case, the combinatorial type is not as tightly constrained as before; the target may expand multiple times, and as such the combinatorial type need not be a bipartite graph with a single interior vertex. Nevertheless, we have the following lemma:
\begin{lemma} 
Let $\Dcal$ be a locus of type IIIb and let $\Delta$ be the corresponding combinatorial type. Then for every nonzero vertex $\diamond$ of the expanded tropical target, the fibre of the tropical map over $\diamond$ contains:
\begin{enumerate}
\item exactly one stable vertex (together with a number of trivial bubbles);
\item at least one vertex which lies on the radius (which may or may not be stable).
\end{enumerate}
\end{lemma}
\begin{proof}
...
Similarly to before, we let $\tau \in \Sigma$ be the ray corresponding to $\Dcal \subseteq \VZ_{1,\alpha}(\PP^N|H,d)$, $\sigma \in \Sigma^{\Kim}$ be the minimal cone containing $\tau$ and $\Ecal \subseteq \ol\Mcal_{1,\alpha}(\PP^N|H,d)$ be the stratum corresponding to $\sigma$, so that we have a map $\Dcal \to \Ecal$ (and $\Ecal$ is the minimal stratum through which the map from $\Dcal$ factors). Recall that we also have a unique stratum $\widetilde\Dcal \subseteq \widetilde\VZ_{1,\alpha}(\PP^N|H,d)$ such that $\Dcal = \widetilde\Dcal \cap \VZ_{1,\alpha}(\PP^N|H,d)$. Note that the cone $\sigma$ has dimension equal to the number of levels of the target expansion in the combinatorial type, which in this case may well be strictly larger than $1$. 
...
Notice that the actual dimension of $\Ecal$ will typically be larger than that of $\Dcal$, since $\Ecal$ is contained inside the excess components of the moduli space. We will describe $\Dcal$ in terms of the map $\Dcal \to \Ecal$, proceeding in two steps:
\begin{enumerate}
\item obtaining $\widetilde\Dcal$ from $\Ecal$;
\item expressing the class of $\Dcal \subseteq \widetilde\Dcal$ in terms of tautological classes.
\end{enumerate}
...
to the locus $\widetilde\Dcal$. In toric geometry, the method for studying this restriction is as follows. The fan of $\widetilde\Dcal$ is obtained by projecting the star of the ray $\tau$ onto the quotient lattice, and similarly for $\Ecal$. The map $\widetilde\Dcal \to \Ecal$ is then a toric morphism, given by the obvious map of fans. In order to describe this morphism, we first subdivide the fan of $\Ecal$ so that every cone in the fan of $\widetilde\Dcal$ maps surjectively onto a cone. This amounts to taking a toric blow-up of $\Ecal$ which we denote by $\widetilde\Ecal$. There is a natural morphism $\widetilde\Dcal \to \widetilde\Ecal$ which is now flat, and this may be described as a compactification of a torus bundle on $\widetilde\Ecal$.

In the logarithmic setting, we proceed as follows. The stratum $\widetilde\Dcal$ has a natural \emph{reduced} logarithmic structure, obtained by restricting the logarithmic structure on $\widetilde\VZ_{1,\alpha}(\PP^N|H,d)$ and then quotienting out by the normal direction to $\widetilde\Dcal$. Similarly, $\Ecal$ has a natural reduced logarithmic structure, which coincides with the pull-back of the product logarithmic structure on the fibre product \eqref{IIIa fibre product}.

We would like to describe the tropicalisations of these logarithmic stacks. In fact, it is more convenient to describe the \emph{extended tropicalisations} \cite{}. Let $l_1,\ldots,l_m$ denote the edge lengths of the tropical target, forming co-ordinates on the cone $\sigma$. Then the extended tropicalisation of $\Ecal$ is obtained by intersecting the star of $\sigma$ with the face $l_1=\ldots=l_m=\infty$.

For $\widetilde\Dcal$ we have on $\tau$ edge lengths $e_1=\ldots=e_r=e$ separating the core from the vertices on the radius. The extended tropicalisation of $\widetilde\Dcal$ is obtained by intersecting the star of $\tau$ with the face $e=\infty$. Since $l_1=\ldots=l_m=\infty$ implies $e=\infty$ there is certainly a morphism between these cone complexes, and it is clear this morphism is a subdivsion.

Moreover we may describe the subdivision explicitly.


\subsubsection{$\Dcal$ from $\widetilde\Dcal$}
