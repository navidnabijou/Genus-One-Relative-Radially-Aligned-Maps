...
\end{enumerate}
subject to the condition that the contraction radius $\delta$ of the central alignment is equal to $\delta_f$, the contraction radius of the stable map $f$. \marginpar{what is the contraction radius of $f$? I think to be precise we need to say $\delta=\lambda(v)$ for $v$ such that $f$ contracts any component of $C$ closer to the circuit than $C_v$, while it has positive degree on $C_v$...} We then define
...
is an effective class in $\Achow_1(Z)$. Since $Z$ is at most genus one and any line bundle of strictly positive degree on a Gorenstein genus one curve is effective, \marginpar{need to look at each irreducible component of $Z$ separately} this condition can be rephrased as the following numerical criterion
