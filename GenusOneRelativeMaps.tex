...
Here $g(v)$ and $d(v)$ are the genus and degree assigned to the vertex, while $Z_v$ is the logarithmic stratum of the target corresponding to the tropical stratum into which the vertex is mapped by the combinatorial type (in our setting, $Z_v=\PP^N$ or $H$). The length of the vector $\alpha(v)$ is given by the number of edges and legs adjacent to $v$, and the entries (which represent tangencies, and may be positive or negative) are given by the expansion factors along those edges and legs. If all of the tangencies are positive, then this is nothing but an ordinary moduli space of log stable maps.

The corresponding logarithmic stratum is then obtained as a follows. For each vertex $v$ and each adjacent edge $E \ni v$ we define $\widetilde\Mcal_v^E$ to be the stack $\ol\Mcal_v$ equipped with the log structure
...
If $E_1,\ldots,E_k$ are the edges adjacent to $v$ then we define
...
\ecd
In the above discussion, there were two steps at which we needed to take fibre products in the fs log category: first, when constructing each $\widetilde{\Mcal}_v$ and second, when construction $\Scal$. Typically the fs fibre product does not agree with the usual fibre product of stacks; rather, one must take the fibre product in the category of coherent log stacks (which does agree with the usual fibre product) and then integralise and saturate the result.

We will now argue that, in our situation, the fibre products of coherent log stacks are automatically integral. This means that the integralisation process is redundant, and so the fs fibre product is obtained from the ordinary fibre product by a process of saturation, which amounts to taking a finite cover of the underlying stack (whose degree we will compute).
...
\begin{equation}\label{DRC condition} f^*[H] - \sum_{i=1}^n \alpha_ix_i =0 \in \Achow_0(C).\end{equation}
...
\[s\colon\OO_{\PP}(-1)\hookrightarrow p^*\left(\mathbb T_F\oplus\bigoplus_{i=1}^s\mathbb T_{\tilde q_i}\right)\xrightarrow{+df_q} f^*(\operatorname{T}\PP^N)_q,\]
