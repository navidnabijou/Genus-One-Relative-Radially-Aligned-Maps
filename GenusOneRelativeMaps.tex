...
If we consider the map $D \colon Q^{r-1} \to Q^r$ given by the $(r-1) \times r$ matrix
\begin{equation*} D= \begin{pmatrix} 1 & 1 & 0 & 0 & \ldots & 0 \\
0 & 1 & 1 & 0 & \ldots & 0 \\
\, & \, & \, &\vdots & \, & \, \\
0 & 0 & \ldots & 0 & 1 & 1 \end{pmatrix}\end{equation*}
then the monoid $P$ can be expressed as a cokernel
\begin{equation*} Q^{r-1} \to \oplus_{i=1}^r Q_{E_i} \to P \end{equation*}
where the first map is obtained by composing $D$ with the natural inclusion $Q^r \to \oplus_{i=1}^r Q_{E_i}$. We may now form a diagram
\bcd
Q^{r-1} \ar[d,"\Id"] \ar[r] & \oplus_{i=1}^r Q_{E_i} \ar[r] \ar[d,hook] & P \ar[d] \\
Q^{r-1} \ar[r] & Q^r \oplus \Z^r \ar[r] & S \oplus \Z^r
\ecd
where both rows are cokernel diagrams and the map $P \to S\oplus\Z^r$ is induced by the universal property. Here $S$ is the cokernel of $D$ and is given by:
\begin{equation*} S = \begin{cases} Q \qquad \text{if } r=1 \\ Q^{\gp} \qquad \text{if } r \geq 2 \end{cases}.\end{equation*}
In any case, $S$ is integral and the map $P \to S \oplus \Z^r$ is injective because the middle vertical maps in injective. Hence we conclude that $P$ is integral, as required. \end{proof}

The previous lemma holds for arbitrary moduli spaces of punctured maps. The next lemma, on the other hand, is special to our setting.
