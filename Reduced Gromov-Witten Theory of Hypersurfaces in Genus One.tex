...
\draw [->,red,thick] (0,-2) -- (3,-2);
\draw [red,fill=red] (2,-2) circle[radius=2pt];
\draw [->,red] (1,-0.9) -- (1,-1.6);

\draw [red] (2,-2) node[above]{\tiny$\diamond$};
\draw [red] (0,-2) node[above]{\tiny$0$};
...
In order to have $\sigma=\RR_{\geq 0}$, the dual graph $\Gamma$ must be a bipartite graph with vertices over $0$ and $\diamond$. The cases $A,B$ and $C^+$ enumerated above cover situations where there is a single vertex mapped to $\diamond$. We claim that if there is more than one such vertex, then the contribution to the recursion is trivial. To see this, we consider the locus
\begin{equation*} \Dcal^{\ACGS} \subseteq \ol\Mcal^{\log}_{1,\alpha}(\PP^N|H,d) \end{equation*}
to which $\Dcal^{\Kim}$ maps under the collapsing map. The codimension of $\Dcal^{\ACGS}$ is given by the dimension of the associated tropical moduli space; if there is more than one vertex mapped to higher level then this is strictly larger than $1$, which is the codimension of $\Dcal^{\Kim}$. Thus the map $\Dcal^{\Kim} \to \Dcal^{\ACGS}$ has positive-dimensional fibres, and since all insertions are pulled back from the latter space the contribution vanishes by the projection formula.\end{proof}
