...
\subsection{Identifying the components of $\Dcal^k$}

\subsubsection{Components of $\Dcal^k$ are log strata} We begin by noting that every irreducible component of $\Dcal^k$ is a codimension--$1$ logarithmic stratum. This is not hard to see: the locus where the log structure is trivial coincides precisely with the  locus where the source curve is smooth and not mapped inside $H$. Moreover since $\VZ$ is log smooth this locus is open and dense. By definition $\Dcal^k$ is disjoint from this locus, hence is contained in the closed complement. Since it has codmension $1$, it must thus be equal to a union of logarithmic strata.

\subsubsection{Describing logarithmic divisors} In the previous section we discussed at length a procedure for determining all the logarithmic strata of $\VZ$. In particular, each divisorial logarithmic stratum is obtained by first choosing the combinatorial type of a tropical map, then subdividing its tropical moduli space, and finally choosing a ray in this subdivision. The logarithmic divisors which are contained in $\Dcal^k$ correspond precisely to the rays of the tropical moduli space where the vertex of the tropicalisation containing $x_k$ is mapped into the interior of $\RR_{\geq 0}$ by the tropical map.

\subsection{Recursive description of the divisors: types $A,B$ and $C^+$}
Let us therefore choose a combinatorial type of a tropical map, corresponding to a logarithmic stratum $\Scal$. In this section we will restrict ourselves to the situation where the minimal subcurve of genus one has positive degree (the other, more complicated, case will be taken up in the next section).

In this situation, the subdivision is trivial since no alignment is necessary (both radii are zero). Following our procedure, we choose a ray of the tropical moduli space $Q^\vee_{\RR}$. This determines a generisation of our original combinatorial type, by setting certain edge lengths to zero. Replace the original combinatorial type with this generisation. In order to have $\Dcal \subseteq \Dcal^k$ we need that the vertex of the tropicalisation which contains the flag $x_k$ is mapped into the interior of $\RR_{\geq 0}$ by the tropical map.

We claim that this can be the only vertex of the tropical curve mapped into the interior of $\RR_{\geq 0}$; if there were more, the positions of their images in $\RR_{\geq 0}$ would be independent and thus the corresponding maximal cone in the tropical moduli space would be at least $2$~dimensional, which is a contradiction since we assumed we had chosen a ray.\footnote{(Luca) Say something about rigid tropical curves as in ACGS and KLR} There are now three cases to distinguish, depending on the position of the circuit. These are illustrated in the figures below.
...
\begin{figure}[h]
    ...
    \end{minipage}
\end{figure}
Note that these are just cartoon pictures: we have omitted the marked points and the expansion factors. The continuity of the tropical map identifies all of the edge lengths, up to weights given by the expansion factors (i.e. tangency orders) at the edges; thus the tropical moduli space is isomorphic to $\RR_{\geq 0}$ and so we have a divisor as expected.

\subsubsection{Recursive description of the divisor}
Let $\Dcal_{\Mcal}^\circ\subseteq \Mcal$ denote the (codimension-$1$) locally closed stratum corresponding to our combinatorial type. The corresponding divisor in $\Mcal$ is $\Dcal_\Mcal = \overline{\Dcal^\circ}$. On the other hand, the corresponding divisor $\Dcal$ in $\VZ$ is the proper transform of $\Dcal_\Mcal$, which is obtained by taking the preimage $\Dcal^\circ$ of $\Dcal_\Mcal^\circ$ (since $\Dcal^\circ$ is disjoint from the blown-up locus) and then taking its closure in $\VZ$. This is different from taking the preimage of $\Dcal_\Mcal$ (the total transform), since in general $\Dcal_\Mcal$ will intersect the blown-up locus. By continuity have an induced map:
\begin{equation*} \Dcal \to \Dcal_\Mcal. \end{equation*}
The remainder of this subsection will be dedicated to showing how the map $\Dcal \to \Dcal_\Mcal$ may be interpreted as a blow-up. This is an extremely useful fact, because $\Dcal_\Mcal$ has a recursive description in terms of moduli spaces of punctured maps \cite{PuncturedMaps}, which in our setting we are able to compute integrals over. We will consider the cases $A,B$ and $C^+$ separately in the next three subsections.


\subsubsection{Type A} Choose an element of $\Dcal=\overline{\Dcal^\circ}$ where $\Dcal^\circ \subseteq \VZ$ is the locus of maps with combinatorial type equal to $\Gamma$, which itself is of type A. We will now argue that such an element restricts to give an aligned map on the piece of the curve corresponding to $\sqC_1$, and that conversely given an aligned map corresponding to $\sqC_1$ and ordinary punctured and logarithmic maps corresponding to $\sqC_0,\sqC_2,\ldots,\sqC_r$, the glued map is aligned. This shows that $\Dcal$ is equal, up to an \'etale quotient, to the fibre product:
\begin{equation*}\Mpunct{0}{\alpha^{(0)}\cup (-m_1,\ldots,-m_r)}{\PP^N|H}{d_0}\times_{H^r}\left(\VZ_{1,\alpha^{(1)}\cup(m_1)}(\PP^N|H,d_1)\times\prod_{i=2}^r\Mlog{0}{\alpha^{(i)}\cup(m_i)}{\PP^N|H}{d_i}\right). \end{equation*}
...
\begin{equation*}\Mpunct{0}{\alpha^{(0)}\cup (-m_1,\ldots,-m_r)}{\PP^N|H}{d_0}\times_{H^r}\prod_{i=1}^r\Mlog{0}{\alpha^{(i)}\cup(m_i)}{\PP^N|H}{d_i}. \end{equation*}
and leaving the rest alone. Note that only the moduli space corresponding to $\sqC_0$ is a moduli space of punctured maps: the rest are usual moduli spaces of log stable maps.
