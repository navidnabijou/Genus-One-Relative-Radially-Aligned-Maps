...
Let $\Dcal$ be a locus of type IIIb with combinatorial type $\Delta$ which contributes nontrivially to the Gromov--Witten invariant. For every nonzero vertex $\diamond$ of the tropical target, the fibre of the tropical map over $\diamond$ contains exactly one stable vertex, which lies on the radius.
\end{lemma}
...
For each vertex of the target there must always be at least one vertex in the preimage which lies on the radius. This is implied by the fact that the combinatorial type $\Delta$ is a ray. In fact, we make the stronger assertion that the stable vertex mapping to $\diamond$ lies on the radius. The alternative is that a vertex dual to a trivial bubble lies on the radius. This latter condition implies that $\Dcal \to \Ecal$ has positive-dimensional fibres, so the contribution vanishes. 
\end{proof}
...
\]
is a logarithmic morphism. The generic fiber of this morphism is toric variety.  \medskip
...
\[
\widetilde\Dcal\to\widetilde \Ecal\to\Ecal,
\]
where the first map is flat and the second map is a logarithmic modification. The generic fiber can be interpreted geometrically. 

{\bf \color{red} This lemma should be there, but the notation needs to be changed and the proof needs to be cleaned up. Luca?
...
We said that $C_i^\prime$ is \emph{a} component of $C_i$ which lies on the radius. There may be more than one, but if this is the case then their tangent spaces are naturally identified. We obtain a natural morphism
