...
where all the $C_i$ are smooth, $C_0$ has genus one, all the other $C_i$ have genus zero, and for $i = 1,\ldots,k$, $C_i$ intersects $C_0$ at a single node (denoted $q_i$) and does not intersect any other components.
...
Fix $i =1,\ldots,k$ and let $m_i$ be the multiplicity of $f$ with $H$ at $q_i \in C_i$. We define:
\begin{equation*} \beta = \prod_{i=1}^k m_i \qquad r_i = \beta/m_i \end{equation*}
...
The $C_i$ are not themselves Cartier, but $r_i  C_i$ is Cartier for $i=1,\ldots,k$. We have:
\begin{equation*} r_i C_i \cdot C_0 = 1 \end{equation*}
Now, let $x_1,\ldots,x_n$ denote the marked points of $C$. These are divisors on the central fibre $W^\prime_0$, and hence can be extended to divisors $\tilde{x}_1,\ldots,\tilde{x}_n$ on $W^\prime$. Consider the line bundle:
\begin{equation*} \tilde{L} = \OO_{W^\prime}(\beta C_0 + \Sigma_{i=1}^n \alpha_i \tilde{x}_i) \end{equation*}
Note that $\beta C_0$ is indeed a Cartier divisor, since
\begin{equation*} \beta C_0 = - \sum_{i=1}^k \beta C_i = - \sum_{i=1}^k m_i (r_i C_i) \end{equation*}
and the $r_i C_i$ are Cartier. We claim that when we restrict $\tilde{L}$ to the central fibre we recover the line bundle $L = f^* \OO(1)$ that we started with.

For $i=1,\ldots,k$, we have:
\begin{align*} \tilde{L}|_{C_i} & = \OO_{C_i} \left( \beta (C_0 \cdot C_i) q_i + \Sigma_{x_j \in C_i} \alpha_j x_j \right) \\
& = \OO_{C_i} \left( (\beta/r_i) q_i + \Sigma_{x_j \in C_i} \alpha_j x_j \right)\\
& = \OO_{C_i} \left( m_i q_i + \Sigma_{x_j \in C_i} \alpha_j x_j \right) \\
& = L|_{C_i} \end{align*}
On the other hand
\begin{align*} \tilde{L}|_{C_0} & = \OO_{C_0} \left( - \sum_{i=1}^k \beta (C_i \cdot C_0) q_i + \sum_{x_j \in C_0} \alpha_j x_j \right) \\
& = \OO_{C_0} \left( - \sum_{i=1}^k m_i q_i + \sum_{x_j \in C_0} \alpha_j x_j \right) \\
& = L|_{C_0} \end{align*}
and the fact that $\tilde{L}|_{W^\prime_0} = L$ follows from the fact that the dual intersection graph of $C$ has genus zero.
