...
%DIF ADD EllipticSingularitiesSplitting.tex         Wed Oct 21 17:34:52 2020
%\linespread{1.25}
...
%DIF 49c76
%DIF < \usetikzlibrary{decorations.pathmorphing,decorations.pathreplacing,patterns,arrows.meta}
%DIF -------
\usetikzlibrary{ipe,decorations.pathmorphing,decorations.pathreplacing,patterns,arrows.meta,shapes.misc} %DIF > 
%DIF -------
...
%DIF < 
%DIF -------
\DeclareRobustCommand{\plE}{\text{\reflectbox{$\exists$}}} %DIF > 
%DIF -------
...
\DIFaddbegin \DIFadd{where $C$ is a nodal genus $1$ curve, $\overline C_1$ and $\overline C_2$ are Gorenstein curves of genus $1$, the map $C\to \PP^N[s]$ is a logarithmic map }\DIFaddend to an $s$-fold expansion of \DIFdelbegin \DIFdel{$\mathbb P^N$ }\DIFdelend \DIFaddbegin \DIFadd{$\PP^N$ }\DIFaddend along $H$\DIFdelbegin \DIFdel{such that the map to the expansion and composite map $C\to \mathbb P^N$ each factor nontrivially through an elliptic singularity. 
}\DIFdelend \DIFaddbegin \DIFadd{, and each of the vertical maps does not contract the minimal subcurve of genus $1$.
}\DIFaddend \end{customthm}

\DIFdelbegin \DIFdel{The moduli space above is constructed by identifying two closed conditions that must be satisfied by the space of maps to the pair $(\mathbb P^N,H)$. Each condition requires the stable map to factor through an elliptic singularity.
The behaviour mixes the geometry in~\mbox{%DIFAUXCMD
\cite{RSPW} }%DIFAUXCMD
and~\mbox{%DIFAUXCMD
\cite{RSPW2}}%DIFAUXCMD
. }%DIFDELCMD < {%%%
\DIFdel{Together with an analysis of the deformation theoryof such maps, we are led to Theorem~\ref{thm: desingularization}}%DIFDELCMD < }%%%
\DIFdel{.
}\DIFdelend %DIF > Intuitively, non-degeneracy is the condition that the maps in question are non-constant on some branch of the minimal genus $1$ subcurve, see  \S \ref{section construction} for %details.

\DIFdelbegin \DIFdel{Our second resultexplains how to do practical calculations on this space of stable maps}\DIFdelend %DIF > The moduli space above is constructed by identifying two closed conditions that must be satisfied by maps to the pair $(\mathbb P^N,H)$. Each condition requires the stable map to factor through an elliptic singularity, and the conditions together are required for the relative and absolute reduced theories to be related appropriately.  {An analysis of the deformation theory of such maps leads to Theorem~\ref{thm: desingularization}}.
\DIFaddbegin 
...
\DIFdelbegin \DIFdel{In addition, the results of~\mbox{%DIFAUXCMD
\cite{RSPW} }%DIFAUXCMD
...
\DIFaddend \subsection{\DIFdelbegin \DIFdel{The absolute case}\DIFdelend \DIFaddbegin \DIFadd{Curves and tropical curves}\DIFaddend } \DIFaddbegin \DIFadd{A nodal curve $C$ over $\spec \mathbb C$ has an associated dual graph $\Gamma(C)$. Given a toric monoid $P$, there is a logarithmic point $\spec \CC_P$ which may be thought of as remembering the information of the embedding of the torus-fixed closed point into the toric variety $\spec \mathbb C[P]$. For a logarithmically smooth curve $C$ over $\spec \CC_P$, the dual graph $\Gamma(C)$ is enhanced: given an edge $e$ of $\Gamma(C)$, the logarithmic structure keeps track of a generalized ``edge length'' $\ell_e$, which is an element of $P$. These data can be repackaged into a cone complex $\plC$ together with a map to the dual cone $\sigma_P$
}\[
...
\DIFadd{The reader is encouraged to think of $\mathfrak M_{g,n}^{\mathrm{trop}}$ as playing the same role for $\mathfrak M_{g,n}$ as a fan plays for a toric variety. In particular, two constructions are relevant:
} \begin{enumerate} 
...
\cite{CavalieriChanUlirschWise}}%DIFAUXCMD
, and we leave these details to the reader. See also \mbox{%DIFAUXCMD
\cite{GrossSiebertLog}}%DIFAUXCMD
.
}
...
\cite{RSPW,RSPW2}}%DIFAUXCMD
. If $C$ is a Gorenstein curve and $f:C\to \mathbb P^N$ is a morphism, the ampleness of the the tangent bundle of $\mathbb P^N$ ensures that if no subcurve of arithmetic genus $1$ is contracted, the deformations of $f$ are unobstructed. In fact, the deformations }\textit{\DIFadd{over the stack of curves}} \DIFadd{are also unobstructed. The moduli space of Gorenstein genus $1$ curves is not smooth. However, a more sophisticated moduli problem, consisting of nodal curves together with a }\textit{\DIFadd{chosen contraction}} \DIFadd{to a Gorenstein curve, might be expected to resolve these singularities. The formation of such a moduli problem requires logarithmic geometry, and is achieved in~\mbox{%DIFAUXCMD
...
\subsection{\DIFadd{Singularities and moduli of attachments}}\label{S:ellsing} \DIFadd{Isolated Gorenstein singularities of genus $1$ were classified by Smyth in \mbox{%DIFAUXCMD
\cite{SMY1}}%DIFAUXCMD
: for every fixed number of branches $m$, there is only one isomorphism type of curve germs with such characteristics. For $m=1,2$ we find the cusp ($A_2$) and tacnode ($A_3$) respectively; for $m\geq 3$ we find the union of $m$ general lines through the origin of $\Aaff^{m-1}$ - Smyth calls them }\emph{\DIFadd{elliptic $m$-fold points}}\DIFadd{.
}

\DIFadd{The problem of constructing an elliptic singularity from its pointed normalisation is more delicate; it was widely studied in van der Wyck's thesis \mbox{%DIFAUXCMD
\cite{vdW}}%DIFAUXCMD
. Recall that the seminormalisation $C^\text{sn}$ of a curve $C$ has the same underlying topological space, but its singularities consist of rational $m$-fold points only, i.e. they are locally isomorphic to the union of the axes in $\Aaff^m$.
}

\begin{lemma}\label{lem:pushout}
 \DIFadd{The seminormalisation $\nu\colon C^\text{sn}\to C$ of an elliptic $m$-fold point is the collapse of a general tangent vector $v$ at the rational $m$-fold point.
}\end{lemma}

\DIFadd{This is equivalent to \mbox{%DIFAUXCMD
\cite[Lemma 2.2]{SMY1}}%DIFAUXCMD
, where he describes the seminormalisation of an elliptic singularity as the data of a hyperplane in the Zariski cotangent space, not containing the coordinate lines. We call }\emph{\DIFadd{moduli of attaching data}} \DIFadd{\mbox{%DIFAUXCMD
\cite[\S 2.2]{SMY2} }%DIFAUXCMD
(or }\emph{\DIFadd{crimping space}}\DIFadd{) the extra data needed to produce a singularity from its pointed normalisation.
}

\begin{cor}
 \DIFadd{The moduli of attachments of an elliptic $m$-fold point is isomorphic to $\mathbb G_{\text{m}}^m$.
}\end{cor}
\DIFadd{One way to compactify it is by allowing }\emph{\DIFadd{sprouting}} \DIFadd{on the boundary, i.e. replacing a branch $R_i$ of the singularity with a strictly semistable component $\PP^1$ with the singularity at $0$ and the branch $R_i$ glued nodally at $\infty$.
}

\DIFadd{There are other ways to look at these data; we recall a construction from linear algebra. Let $V=\sum_{i=1}^mL_i$ be the presentation of a vector space as the direct sum of $m$ lines. It is equivalent to give compatible isomorphisms $\theta_{i,j}\colon L_i\simeq L_j$ and a general line $\ell$ in $V$ (or a hyperplane in the dual vector space $V^\vee$). Indeed, given a general line $\ell$, a vector $v\in\ell$ can be written as:
}\[\DIFadd{v=a_1e_1+\ldots+a_me_m,\qquad a_i\neq 0\ \forall i=1,\ldots,m,}\]
\DIFadd{for a fixed basis $\{e_i\in L_i\}$ of $V$. We can then define $\theta_{i,j}$ by:
}\[\DIFadd{\theta_{i,j}(e_i)=\frac{a_j}{a_i}e_j;}\]
\DIFadd{it is easy to see that it does not depend on any choice made. On the other hand, given the $\theta_{i,j}$, the line $\ell$ can be taken to be the graph of the morphism:
}\[\DIFadd{(\theta_{1,2},\ldots,\theta_{1,m})\colon L_1\to L_2\oplus\ldots\oplus L_m.}\]

\DIFadd{Lemma \ref{lem:pushout} provides a valuable characterisation of maps from an elliptic singularity $C$, that we are going to exploit in \S \ref{section reduced splitting}:
}\begin{cor}\label{cor:maps_from_elliptic_sing}
 \DIFadd{A map $C\to X$ is the same as a map $C^\text{sn}\to X$ such that its differential sends $v$ to $0$.
}\end{cor}

\DIFadd{Line bundles on an elliptic $m$-fold point admit a similar description, see e.g. \mbox{%DIFAUXCMD
\cite[Lemma 7.5.12]{LiuQ}}%DIFAUXCMD
.
}\begin{lemma}\label{lem:PicE}
 \DIFadd{Let $C$ be a curve of arithmetic genus $1$ containing an elliptic $m$-fold point. Then $\operatorname{Pic}^0(C)\simeq\mathbb G_{\text{a}}$.
}\end{lemma}


\subsection{\DIFadd{Absolute geometry: reminders}} \DIFaddend Let $\mathfrak M_{1,n}$ be the logarithmic algebraic stack of $n$-pointed prestable curves of genus one. We begin by recalling the logarithmic moduli spaces of genus one curves constructed in~\cite[\S 2 \& \S 4]{RSPW}. 
\DIFdelbegin \DIFdel{Given }\DIFdelend \DIFaddbegin 

\DIFadd{Consider }\DIFaddend a logarithmically smooth curve $C$ over a logarithmic geometric point $S$ \DIFdelbegin \DIFdel{, let $\plC$ }\DIFdelend \DIFaddbegin \DIFadd{with monoid $P$ and let $\pi\colon \plC\to \sigma_P$ }\DIFaddend denote its tropicalization\DIFdelbegin \DIFdel{, and let $\lambda \in \overline{M}_{S}$ be the piecewise-linear function giving the distance from each vertex of $\sqC$ to the minimal genus one subcurve. }\DIFdelend \DIFaddbegin \DIFadd{. Given a point $t$ in $\sigma_P$ we obtain a metric graph as explained above. Since the curve in question has genus $1$, there is a minimal genus $1$ subgraph -- either a vertex of genus $1$ or a cycle in the graph. We will refer to this as the }\textbf{\DIFadd{core}}\DIFadd{, denoted by $\plE$ below.
}

\DIFadd{Given a point $s$ with $\pi(s)$ equal to $t$, there is a well-defined distance in the metric graph $\pi^{-1}(t)$ from $s$ to the core. 
In particular, for each vertex $v$ of the dual graph $\Gamma(C)$, i.e. each component of $C$, there is a well-defined function $\lambda(v): \sigma_P\to \RR_{\geq 0}$, defined on the base of the tropical family. We may as well think of $\boldsymbol{\lambda}$ as a piecewise linear function $\lambda\colon\plC\to\RR_{\geq 0}\times\sigma_P$, see Figure \ref{fig:lambdaexample}. Given any other linear function $\delta$ on $\sigma_P$, say that $\delta$ is }\textbf{\DIFadd{comparable}} \DIFadd{to $\lambda(v)$ if there is a global order relation, either $\lambda(v)\leq \delta$ or $\lambda(v)\geq \delta$, that holds at all points of $\sigma_P$. 
}

\DIFadd{Piecewise linear functions on $\plC$ are naturally identified with global sections of the characteristic sheaf $\overline M_C$; linear functions on $\sigma_P$ are naturally identified with global sections of the characteristic sheaf $\overline M_S$, and can be pulled back to $C$. Given an arbitrary logarithmically smooth curve $C/S$, conditions involving the comparability of piecewise linear functions can be imposed at the level of geometric points; they determine subdivisions of $\sigma_P$, and, in turn, logarithmic modifications of $S$. %DIF > This leads to the following definition. 
}
...
\DIFaddbegin \begin{figure}
 ...
 \end{tikzpicture}
\caption{\DIFaddFL{The function $\lambda$ (distance from the core) and its cutoff value $\delta$.}}
\label{fig:lambdaexample}
\end{figure}



\DIFaddend The moduli stack $\mathfrak M_{1,n}^{\mathrm{cen}}$  of centrally aligned curves is a logarithmic algebraic stack in the smooth topology, and $\mathfrak M_{1,n}^{\mathrm{cen}}\to\mathfrak M_{1,n}$ is a logarithmic modification. The main construction of~\cite{RSPW} canonically associates to any centrally aligned family of curves $\mathcal C_S$ a partial destabilisation $\widetilde{\mathcal C}_S$\DIFaddbegin \DIFadd{, introducing $2$-pointed components, }\DIFaddend and a contraction
\[
\widetilde{\mathcal C}_S\to \overline{\mathcal C}_S,
\]
where $\overline{\mathcal C}_S$ may contain a Gorenstein elliptic singularity \cite{SMY1}. \DIFdelbegin \DIFdel{The number of branches }\DIFdelend \DIFaddbegin \DIFadd{Given a family of tropical curves, the function $\delta$ provides a non-negative real number for each point on the base. We may therefore consider the ``}\textbf{\DIFadd{circle}}\DIFadd{'' of points on each fiber whose distance from the core }\DIFaddend is equal to \DIFdelbegin \DIFdel{the sum of }\DIFdelend \DIFaddbegin \DIFadd{$\delta$. A partial destabilization, or ``bubbling'', introduces $2$-pointed rational curves into the curve by adding vertices to the dual graph at the points that lie on this circle (the crosses at height $\delta$ in Figure \ref{fig:lambdaexample}); we denote this new tropical curve by $\widetilde{\plC}$. The open disc of radius $\delta$, i.e. the locus where $\lambda<\delta$, is then contracted to a Gorenstein singularity in $\overline{\mathcal C}_S$. The number of branches is equal to }\DIFaddend the number of vertices of $\widetilde{\sqC}$ which lie on the circle\DIFdelbegin \DIFdel{of radius $\delta$. This data uniquely determines the singularity. }\DIFdelend \DIFaddbegin \DIFadd{. %DIF > This data uniquely determines the singularity.
}\DIFaddend 

The space of stable maps $\widetilde\VZ_{1,n}(\PP^N,d)$ from the universal nodal curve $\mathcal C$ over the stack $\mathfrak M_{1,n}^{\mathrm{cen}}$ is \DIFdelbegin \DIFdel{proper and algebraic}\DIFdelend \DIFaddbegin \DIFadd{Deligne-Mumford and proper}\DIFaddend , with projective coarse moduli space. A compatibility condition between the alignment and the map is required, namely that the circle of radius $\delta$ passes through \DIFdelbegin \DIFdel{at least one component of positive degree}\DIFdelend \DIFaddbegin \emph{\DIFadd{at least one component of positive degree}}\DIFaddend . A stable map in this space is said to satisfy the \textbf{factorisation condition} if the map \DIFdelbegin \DIFdel{$\mathcal C\to \mathbb P^N$ }\DIFdelend \DIFaddbegin \DIFadd{$\widetilde{\mathcal C}\to \mathcal C\to \mathbb P^N$ }\DIFaddend factors through the contraction $\widetilde{\mathcal C}\to \overline{\mathcal C}$. \DIFaddbegin \DIFadd{Maps that are not constant on any genus $1$ subcurve of $\mathcal C$ automatically satisfy this condition; indeed, the contraction is an isomorphism in this case.
 }\DIFaddend 
...
\subsection{Relative geometry: compactification} Fix a hyperplane $H\subseteq \mathbb P^N$. Let $\alpha$ be a partition of the degree $d>0$. Consider the moduli space $\mathcal M_{1,\alpha}^\circ(\mathbb P^N|H,d)$ of maps from smooth \DIFdelbegin \DIFdel{elliptic }\DIFdelend \DIFaddbegin \DIFadd{genus $1$ }\DIFaddend curves $C\to \mathbb P^N$ that meet $H$ at finitely many marked points with vanishing orders given by the partition $\alpha$. This is a smooth, but non-proper Deligne--Mumford stack. 
...
In order \DIFaddbegin \DIFadd{to }\DIFaddend make the obstruction theory more geometric, we expand the target. To elucidate the connection with the static target, consider a logarithmic stable map $[C\to (\mathbb P^N,H)]$ over $\mathrm{Spec} \ (\mathbb N\to \mathbb C)$. At the level of tropicalizations, there is a map of \DIFdelbegin \DIFdel{fans
}\DIFdelend \DIFaddbegin \DIFadd{cone complexes:
}\DIFaddend \[
\plC\to \mathbb R_{\geq 0}.
\]
Choose a subdivision of $\mathbb R_{\geq 0}$ whose vertices \DIFdelbegin \DIFdel{consist of }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend the images of vertices of $\plC$. Pull this subdivision back to $\plC$ by \DIFdelbegin \DIFdel{marking all preimages }\DIFdelend \DIFaddbegin \DIFadd{subdividing it along the preimage }\DIFaddend of the vertices of $\mathbb R_{\geq 0}$. Denote the resulting map $\widetilde \plC \to \widetilde{\mathbb R}_{\geq 0}$. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend These subdivisions induce logarithmic modifications\DIFaddbegin \DIFadd{:
}\DIFaddend \[
...
Let $D_F\subseteq C$ be the maximal genus \DIFdelbegin \DIFdel{one }\DIFdelend \DIFaddbegin \DIFadd{$1$ }\DIFaddend subcurve which is mapped to a higher level and contracted by the map $C\to \mathbb P^N[s]$. Let $\delta_F$ denote the distance from the core to the nearest component of $C \setminus D_F$. Similarly let $D_B\subseteq C$ be the maximal genus \DIFdelbegin \DIFdel{one }\DIFdelend \DIFaddbegin \DIFadd{$1$ }\DIFaddend subcurve that is contracted by the collapsed map to $\mathbb P^N$\DIFaddbegin \DIFadd{, }\DIFaddend and let $\delta_B$ be the associated radius. This coincides with the radius of the underlying map to $\mathbb P^N$. Of course, $\delta_F\leq \delta_B$, so $(C,\delta_F)$ is centrally aligned \DIFdelbegin \DIFdel{also}\DIFdelend \DIFaddbegin \DIFadd{as well}\DIFaddend .
...
%DIF > A Gorenstein elliptic singularity has a number of \textbf{branches}. Given a curve $\overline C$ of genus $1$ with such a singularity, define a branch of $\overline C$ to be an irreducible component containing a branch of the singularity. If the curve $C$ is nodal, then its branches will be understood to be the components of its minimal arithmetic genus $1$ subcurve, i.e. its core.
\DIFaddbegin \DIFadd{Recall that we name }\emph{\DIFadd{core}} \DIFadd{the minimal genus $1$ subcurve of a Gorenstein curve of arithmetic genus $1$, so the latter decomposes into the nodal union of the core and a (possibly empty) rational forest. If the curve contains an elliptic $m$-fold point, we say a map is non-constant on the core if at least one branch of the singularity is not contracted.
}\DIFaddend The following condition identifies the locus of smoothable maps. 

\begin{definition}
The map \DIFdelbegin \DIFdel{$[C\to \mathbb P^N[s]\to \mathbb P^N]$ }\DIFdelend \DIFaddbegin \DIFadd{$f\colon C\to \mathbb P^N[s]$ }\DIFaddend \textbf{\DIFdelbegin \DIFdel{factorises }\DIFdelend \DIFaddbegin \DIFadd{factors }\DIFaddend completely} if 
 \begin{enumerate} 
\item \DIFdelbegin \DIFdel{the map $C\to \mathbb P^N[s]$ }\DIFdelend \DIFaddbegin \DIFadd{$f$ }\DIFaddend factors through $\overline C_F$ \DIFdelbegin \DIFdel{such that at least one branch of $\overline C_F$ has positive degree in the fibre direction; 
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{the collapsed map to $\mathbb P^N$ factorises }\DIFdelend \DIFaddbegin \DIFadd{and it is non-constant on its core,
}\item \DIFadd{letting $f_B\colon C\to\PP^N$ denote the composite of $f$ with the collapsing map $\mathbb P^N[s]\to\mathbb P^N$, the map $f_B$ factors }\DIFaddend through $\overline C_B$ \DIFdelbegin \DIFdel{such that at least one branch of $\overline C_B$ has positive degree}\DIFdelend \DIFaddbegin \DIFadd{and it is non-constant on its core}\DIFaddend .
 \end{enumerate} 
\end{definition}

\DIFdelbegin \DIFdel{In particular, if }\DIFdelend \DIFaddbegin \begin{rem}
 \DIFadd{The factorisation conditions are understood to be satisfied when $C\to \mathbb P^N[s]$ (resp. $C\to \mathbb P^N$) does not contract a genus $1$ subcurve. 
}\end{rem}

\DIFadd{Let
}\begin{equation*} \DIFadd{\VZ_{1,\alpha}(\mathbb P^N|H,d) \subseteq \widetilde\VZ_{1,\alpha}(\PP^N|H,d) }\end{equation*}
\DIFadd{be the stack of maps from centrally aligned curves to expansions that factor completely.
}

\DIFadd{If }\DIFaddend $[C\to \mathbb P^N[s]\to \mathbb P^N]$ is a family of centrally aligned maps over $S$ that \DIFdelbegin \DIFdel{factorises }\DIFdelend \DIFaddbegin \DIFadd{factors }\DIFaddend completely, there is a \DIFdelbegin \DIFdel{forgetful }\DIFdelend moduli map $S\to \VZ_{1,n}(\mathbb P^N,d)$, to the principal component of the space of absolute maps. 
\DIFdelbegin \DIFdel{Let
}\begin{displaymath} \DIFdel{\VZ_{1,\alpha}(\mathbb P^N|H,d) \subseteq \widetilde\VZ_{1,\alpha}(\PP^N|H,d) }\end{displaymath}
%DIFAUXCMD
\DIFdel{be the stack of maps from centrally aligned curves to expansions of $\mathbb P^N$ that factorise completely.
}\DIFdelend 
...
\DIFaddend which remembers only the stabilisation of the collapsed map. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{To see that such a forgetful morphism exists, we have already observed that, for a map $[C\to \mathbb P^N[s]\to \mathbb P^N]$, the stabilised composite }\DIFaddend map is certainly centrally aligned\DIFdelbegin \DIFdel{, and we have already argued above that }\DIFdelend \DIFaddbegin \DIFadd{; moreover, }\DIFaddend it satisfies the factorisation \DIFdelbegin \DIFdel{property for $\mathbb P^N$ after composition.
}\DIFdelend \DIFaddbegin \DIFadd{condition by assumption.
}

\DIFaddend The moduli space $\VZ_{1,\alpha}(\mathbb P^N|H,d)$ is proper. Indeed, the verification of the valuative criterion follows \DIFdelbegin \DIFdel{exactly as in }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend Vakil's Lemma~\cite[Lemma~5.9]{Vre}. A direct proof that the factorisation property is a closed condition may be found in~\cite[Theorem~4.3]{RSPW}.
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{On restricting to the minimal genus $1$ subcurve $D_F$, the }\DIFaddend line bundle $\mathcal O_{C}(-\alpha)$ is equivalent to the divisor $\sum a_i x_i$ where $x_i$ are the points connecting $D_F$ to the rest of the curve, and $a_i$ are the slopes of $\alpha$ along the edges corresponding to the $x_i$. Infinitesimally moving the point $x_i$ \DIFdelbegin \DIFdel{is a }\DIFdelend \DIFaddbegin \DIFadd{produces a nontrivial deformation of the line bundle $\mathcal O_{C}(-\alpha)$. The moduli space $\VZ_{1,n}(\mathbb P^N,d)$ is a logarithmically smooth fibration over the universal Picard stack over the stack of centrally aligned $n$-pointed genus $1$ curves~\mbox{%DIFAUXCMD
\cite[Remark~4.5.3]{RSPW}}%DIFAUXCMD
. As a consequence, it is possible to find a logarithmic }\DIFaddend deformation of the \DIFdelbegin \DIFdel{curve that is unobstructed by the map $f_B$, since the }\DIFdelend map \DIFdelbegin \DIFdel{is constant on the interior of the circle of radius $\delta_B$. By deforming the $x_i$, at least one of which has nonzero $a_i$, we produce a one-dimensional space of obstructions. The absolute obstructions therefore vanish, so the result follows. }\DIFdelend \DIFaddbegin \DIFadd{$(C,f_B)$ that induces the deformation of the line bundle that is given by infinitesimal motion. The obstruction group is $1$-dimensional, and we have produced a logarithmic deformation of the collapsed map that produces a nontrivial deformation of the line bundle $\mathcal O_{C}(-\alpha)$. Therefore, logarithmic deformations surject onto the obstruction space above. The absolute obstructions, which are the cokernel of this map therefore vanish, and the result follows.  
}\DIFaddend %Consider an $S$-family of expanded maps $[F: C\to \mathbb P^N[s]$. Choose $N$ generic hyperplanes in and let $\Delta = \{H\}\cup\{H_1,\ldots,H_N}$ be this set of hyperplanes. There is a morphism of logarithmic schemes $(\mathbb P^N,\Delta)\to (\mathbb P^N,H)$. This induces a new expansion $\mathbb P^N[\Delta,s]$. Moreover, by the genericity of the $H_i$, they each intersect the image of $C$ in finitely many reduced points. Pulling back the logarithmic structure, we obtain a new logarithmic map 
%\[
...
\DIFaddbegin \DIFadd{\textcolor{red}{luca's rewrite of log smoothness}
}

\DIFadd{The proof is analogous to that of \mbox{%DIFAUXCMD
\cite[Theorem 3.5.1]{RSPW2}}%DIFAUXCMD
. There is a forgetful morphism
}\[
\DIFadd{\nu\colon \VZ_{1,\alpha}(\mathbb P^N|H,d)\to \VZ_{1,n}(\mathbb P^N,d),
}\]
\DIFadd{remembering only the stabilisation of the composite $f_B\colon C\to\PP^N[s]\to\PP^N$. A priori, this is in $\overline{\pazocal M}_1(\PP^N,d)$, but in fact by assumption $(C,\delta_B)$ is centrally aligned, and $f_B$ factors through $\overline C_B$, as we have already remarked.
}

\DIFadd{The expanded target $\PP^N[s]$ is logarithmically \'etale over $\PP^N$ with the divisorial log structure associated to $H$, so it is equivalent to study the deformation theory of the unexpanded map to $(\PP^N,H)$. The logarithmic morphism $(\PP^N,H)\to \PP^N$ induces an exact sequence of log tangent bundles:
}\[\DIFadd{0\to T_{\PP^N}(-\operatorname{log}H)\to T_{\PP^N}\to\OO_H(H)\to 0,}\]
\DIFadd{relating the absolute and relative obstruction theories. This shows that $\nu$ can be obstructed only when a genus one subcurve is mapped to higher level in such a way that it is contracted by the collapsed map $f_B$. In this case, let $C_0$ denote the core, and let $E$ be the connected component of $f_B^{-1}(f_B(C_0))$ of arithmetic genus one.
}

\DIFadd{Let $f\colon C\to\PP^N[s]$ over the base $S$ be a possibly obstructed map to the expansion that factors completely, and let $S^\prime$ be a strict square-zero extension of the base.  The deformation problem is local, so we may as well suppose that $\overline M_S^{\text{gp}}$ is a constant sheaf of monoids with stalk $\overline M_{S,s}^{\text{gp}}=:Q$, for a geometric point $s\in S$. Note that sections of $M_S$ such as $\delta_B$ and $\delta_F$ extend automatically due to strictness. Moreover, it is equivalent to deform $f$ or $f_F\colon\overline C_F\to\PP^N_S$, where $\overline C_F$ is given the trivial logarithmic structure around the elliptic singularity.
}

\DIFadd{The map $f_F\colon \overline C_F\to\PP^N[s]$ induces a tropical map $\alpha\colon\plC_F\to\RR_{\geq 0}\times\sigma_Q$, that we want to think of as a section $\alpha\in H^0(\overline C_F,\overline M_C^{\text{gp}})$. With notation as above, we obtain a trivialisation of the line bundle $\OO_{\overline C_F}(-\alpha)$ upon restriction to $E$. By \mbox{%DIFAUXCMD
\cite[Proposition 2.4.1]{RSPW}}%DIFAUXCMD
, the latter is isomorphic to $\OO_E(\sum\mu_i x_i)$, where $\mu_i$ is the slope/expansion factor of $\alpha$ along the edge $e_i$ of $\plC_F$, and $x_i$ the node corresponding to $e_i$ thought of as a smooth marking on $E$. 
}

\DIFadd{The deformation problem for $f_F$ relative to $(C^\prime,\delta_B,f_B^\prime)$ reduces to the problem of extending $\alpha$ as a trivialisation of $\OO_E(\sum\mu_i x_i)$. This gives an interpretation of $\operatorname{Obs}(f/(C,\delta_B,f_B))$ as $H^1(\overline C_F,\OO)$. The latter is a vector space of dimension $1$. Associated to the forgetful morphism $\nu$ at the level of perfect obstruction theories, we find a map:
}\begin{equation}\DIFadd{\label{eqn:def-obs}\operatorname{Def}(C,\delta_B,f_B)\to \operatorname{Obs}(f/(C,\delta_B,f_B)).}\end{equation}
\DIFadd{The left-hand side contains in particular the space $\operatorname{Def}(E,\mathbf{x})=\operatorname{Ext}^1(\Omega_E(\mathbf x),\OO_E)$, since $E$ is contracted by $f_B$. Since by assumption at least one of the $\mu_i$ is non-zero, $\operatorname{Ext}^1(\Omega_E(\mathbf x),\OO_E)$ surjects onto $H^1(\overline C_F,\OO)$ by \mbox{%DIFAUXCMD
\cite[Corollary 3.5.3]{RSPW2}}%DIFAUXCMD
. Looking back at }\eqref{eqn:def-obs}\DIFadd{, we see that obstructions to deforming $f$ can be killed by an appropriate choice of lift $(C^\prime,\delta_B,f_B^\prime)$.
}


\DIFaddend \end{proof}

The \DIFdelbegin \DIFdel{above }\DIFdelend theorem guarantees that the singularities of the space of \DIFdelbegin \DIFdel{totally factorised }\DIFdelend \DIFaddbegin \DIFadd{completely factoring }\DIFaddend maps to expansions is logarithmically smooth. \DIFdelbegin \DIFdel{In fact, one }\DIFdelend \DIFaddbegin \DIFadd{One }\DIFaddend can say more in this case.
...
\DIFaddbegin \DIFadd{\textcolor{red}{Luca's rewrite of the Corollary: Up to a generalised root construction, $\VZ_{1,\alpha}(\mathbb P^N|H,d)$ is a smooth Deligne-Mumford stack.}
}
...
\subsection{Rubber variants}\label{subsection rubber} \DIFdelbegin \DIFdel{In our implementation of the relative theory, we will need the }\DIFdelend \DIFaddbegin \DIFadd{We require a }\DIFaddend ``rubber'' variant of the space of maps \DIFdelbegin \DIFdel{$
\VZ_{1,\alpha}(\mathbb P^N|H,d)$}\DIFdelend \DIFaddbegin \DIFadd{$\VZ_{1,\alpha}(\mathbb P^N|H,d)$}\DIFaddend , where the \DIFdelbegin \DIFdel{entire curve is mapped into }\DIFdelend \DIFaddbegin \DIFadd{curve is contained in the }\DIFaddend higher levels.

\DIFdelbegin \DIFdel{Specifically, let }\DIFdelend \DIFaddbegin \DIFadd{Let }\DIFaddend $\mathbb P$ denote the projective bundle $\mathbb P(\mathcal O(1)\oplus\mathcal O)$ on \DIFdelbegin \DIFdel{$\mathbb P^{N-1}$}\DIFdelend \DIFaddbegin \DIFadd{$H\simeq\mathbb P^{N-1}$}\DIFaddend . Equip this space with the logarithmic structure coming from the $0$ and $\infty$ sections of the bundle. Consider the moduli space $\VZ^{\leftrightarrow}_{1,\alpha}(\mathbb P|H_0+H_\infty,d)$ of logarithmic maps
\[
C\to \mathbb P[s]\to \mathbb P^{N-1},
\]
which \DIFdelbegin \DIFdel{factorise }\DIFdelend \DIFaddbegin \DIFadd{factor }\DIFaddend completely, and where automorphisms are \DIFdelbegin \DIFdel{considered over the space of maps }\DIFdelend \DIFaddbegin \DIFadd{taken to cover the identity on the map }\DIFaddend to $\mathbb P^{N-1}$\DIFdelbegin \DIFdel{. Specifically, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend two maps that differ by a $\mathbb G_{\operatorname{m}}$ translate in the fibre direction are considered equivalent. \DIFdelbegin \DIFdel{Total factorisation, similar to the previous case, means that }\DIFdelend \DIFaddbegin \DIFadd{As previously, }\DIFaddend the map to the bundle and the map to the base $\mathbb P^{N-1}$ both \DIFdelbegin \DIFdel{factorise }\DIFdelend \DIFaddbegin \DIFadd{factor }\DIFaddend through possibly different singularities. 
...
obtained by performing a logarithmic modification and \DIFdelbegin \DIFdel{then }\DIFdelend passing to a closed substack \DIFaddbegin \DIFadd{where the factorisation property is satisfied}\DIFaddend . This moduli space will \DIFdelbegin \DIFdel{not in general be logarithmically smooth}\DIFdelend \DIFaddbegin \DIFadd{typically be non-equidimensional}\DIFaddend , but we \DIFdelbegin \DIFdel{may }\DIFdelend equip it with a virtual class as follows. The \DIFdelbegin \DIFdel{complete linear system $|\OO_X(Y)|$ }\DIFdelend \DIFaddbegin \DIFadd{divisor $Y$ }\DIFaddend defines an embedding $X \hookrightarrow \PP^N$\DIFaddbegin \DIFadd{, }\DIFaddend with $Y=X\cap H$ for \DIFdelbegin \DIFdel{$H$ some hyperplane }\DIFdelend \DIFaddbegin \DIFadd{some hyperplane $H$}\DIFaddend .
...
Since $\VZ_{1,n}(\PP^N,d)$ is smooth and $\VZ_{1,n}(X,\beta)$ carries a natural virtual class \cite[Theorem 4.4.1]{RSPW}\DIFdelbegin \DIFdel{there is a diagonal pull-back morphism~\mbox{%DIFAUXCMD
\cite[Appendix C]{BattistellaNabijou}}%DIFAUXCMD
, which we use to define }\DIFdelend \DIFaddbegin \DIFadd{, the diagonal of $\VZ_{1,n}(\PP^N,d)$ defines a pullback~\mbox{%DIFAUXCMD
\cite[Appendix A]{BattistellaNabijou}}%DIFAUXCMD
, and therefore }\DIFaddend the virtual class on the space of maps to $(X,Y)$:
\begin{equation*} \virt{\VZ_{1,\alpha}(X|Y,\beta)} := i_\Delta^! [\VZ_{1,\alpha}(\PP^N|H,d)]. \end{equation*}
\DIFdelbegin \DIFdel{Since }\DIFdelend \DIFaddbegin \DIFadd{Note that this is parallel to the virtual formalism in~\mbox{%DIFAUXCMD
\cite{Ga}}%DIFAUXCMD
...
\noindent The logarithmic smoothness \DIFdelbegin \DIFdel{established in }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend Theorem~\ref{thm: log-smoothness} implies that $\VZ_{1,\alpha}(\mathbb P^N|H,d)$ is an orbifold toroidal embedding. \DIFdelbegin \DIFdel{Consequently, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend irreducible components of its boundary \DIFdelbegin \DIFdel{and their intersections form a stratification of the space. This stratification will be important in the sequel}\DIFdelend \DIFaddbegin \DIFadd{stratify the space, as we now describe}\DIFaddend .

The tropicalizations of logarithmic maps that \DIFdelbegin \DIFdel{factorise completely satisfy a natural combinatorial }\DIFdelend \DIFaddbegin \DIFadd{factor completely are combinatoriall constrained by a }\DIFaddend condition known as \DIFdelbegin \DIFdel{well-spacedness}\DIFdelend \DIFaddbegin \textbf{\DIFadd{well-spacedness}}\DIFaddend . The version we use here is essentially identical to~
\cite{RSPW2}. 

\begin{definition}
Let $\plC$ be a tropical curve of genus one and let \DIFdelbegin \DIFdel{$\plC_0$ }\DIFdelend \DIFaddbegin \DIFadd{$\plE$ }\DIFaddend be its minimal subcurve of genus one. A tropical map $F: \plC\to \mathbb R_{\geq 0}$ is said to be \textbf{well-spaced} if one of the following two conditions are satisfied:
 \begin{enumerate} 
    \item no open neighbourhood of \DIFdelbegin \DIFdel{$\plC_0$ }\DIFdelend \DIFaddbegin \DIFadd{$\plE$ }\DIFaddend is contracted to a point in $\mathbb R_{>0}$, or
    \item\DIFaddbegin \label{pt:well-spaced2} \DIFaddend if an open neighbourhood of \DIFdelbegin \DIFdel{$\plC_0$ }\DIFdelend \DIFaddbegin \DIFadd{$\plE$ }\DIFaddend is contracted and $t_1,\ldots,t_k$ are the flags whose vertex is mapped to \DIFdelbegin \DIFdel{$F(\plC_0)$}\DIFdelend \DIFaddbegin \DIFadd{$F(\plE)$}\DIFaddend , but along which $F$ has nonzero slope; then, the minimum of the distances from \DIFdelbegin \DIFdel{$\plC_0$ to }\DIFdelend \DIFaddbegin \DIFadd{$\plE$ to a vertex supporting }\DIFaddend $t_i$ occurs for at least two indices $i$.
 \end{enumerate} 
\end{definition}
...
\section{Degeneration of contact order}\label{Section Gathmann line bundle} Consider \DIFdelbegin \DIFdel{a reduced relative }\DIFdelend \DIFaddbegin \DIFadd{the moduli }\DIFaddend space $\VZ_{1,\alpha}(\PP^N|H,d)$ \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{of relative maps that factor completely.
}\begin{dfn}
 \DIFaddend For each marking $x_k$\DIFdelbegin \DIFdel{we }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend consider the locus $\Dcal_{1,\alpha,k}(\PP^N|H,d) \subseteq \VZ_{1,\alpha}(\PP^N|H,d)$ where $x_k$ belongs to an \DIFdelbegin \DIFdel{internal component }\DIFdelend \DIFaddbegin \textbf{\DIFadd{internal component}} \DIFaddend of the collapsed map\DIFaddbegin \DIFadd{, i.e. a component of the curve that is mapped into $H$}\DIFaddend .
\DIFaddbegin \end{dfn}
\noindent \DIFadd{Equivalently, for the expanded map, $x_k$ does not belong to a }\emph{\DIFadd{trivial bubble}} \DIFadd{component.
}

 \DIFaddend In this section, we use the logarithmic structure on $\VZ_{1,\alpha}(\PP^N|H,d)$ to construct a line bundle $\Lcal_k$ \DIFdelbegin \DIFdel{together with a }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend section $s_k$ which vanishes precisely along $\Dcal_{1,\alpha,k}(\PP^N|H,d)$. We use the correspondence with tropical geometry to identify $\cchern_1(\Lcal_k)$ in terms of tautological classes on $\VZ_{1,\alpha}(\PP^N|H,d)$, and to compute the vanishing order of $s_k$ along the components of $\Dcal_{1,\alpha,k}(\PP^N|H,d)$. Combined with the relative splitting \DIFdelbegin \DIFdel{axioms established in }\DIFdelend \DIFaddbegin \DIFadd{formulas from }\DIFaddend the next section, we obtain a recursion relation inside $\VZ_{1,\alpha}(\PP^N|H,d)$
...
\DIFdelbegin \DIFdel{. 
}\DIFdelend in terms of fibre products of moduli spaces with smaller numerical data. The tautological description of the factorisation condition then leads to a recursive structure for calculating integrals. This \DIFdelbegin \DIFdel{last }\DIFdelend step is the heart of the analysis. 
...
\noindent \DIFaddbegin \DIFadd{where black vertices represent rational components, white vertices represent genus one. }\DIFaddend The degrees of vertices, expansions factors, and remaining markings are distributed arbitrarily, subject to the following constraints:
 \begin{enumerate} 
...
\subsubsection{Possible combinatorial types}\DIFaddbegin \label{S:combinatorialdescription} \DIFaddend A boundary divisor $\Dcal \subseteq \Dcal_{1,\alpha,k}(\PP^N|H,d)$ is said to have \textbf{type $\dag$} if the core is assigned degree zero by the combinatorial type. Divisors of this type occur as principal components in compactified torus bundles over strata in the space of relative maps. As a result, their combinatorial types exhibit very degenerate behaviour. Precisely, over the generic point of such a divisor, the target may expand multiple times, and the source tropical curve need not be bipartite with a single interior vertex\DIFaddbegin \DIFadd{; see the left-hand side of Figure \ref{fig:off_we_go} for a concrete example}\DIFaddend . We stress once again that such degenerate contributions are at the heart of the reduced theory and cannot be removed.
\DIFaddbegin 

\DIFaddend We begin with \DIFdelbegin \DIFdel{the following lemma:
}%DIFDELCMD < \begin{lemma} %DIFDELCMD < \label{lemma type C0 combinatorial types}%%%
%DIFDELCMD < %%%
\DIFdel{Let }\DIFdelend \DIFaddbegin \DIFadd{a series of lemmas describing the combinatorial types of rays associated to divisorial loci }\DIFaddend $\Dcal$ \DIFdelbegin \DIFdel{be a locus }\DIFdelend of type $\dag$ \DIFdelbegin \DIFdel{with combinatorial type $\Delta$ which contributes }\DIFdelend \DIFaddbegin \DIFadd{contributing }\DIFaddend nontrivially to the Gromov--Witten \DIFdelbegin \DIFdel{invariant.
}\DIFdelend \DIFaddbegin \DIFadd{invariants.
}\begin{lemma} \label{lemma type C0 combinatorial types}
 \DIFaddend For every nonzero vertex $\diamond$ of the tropical target, the fibre of the tropical map over $\diamond$ contains exactly one stable vertex, which either is the core, or lies on the circle.
\DIFdelbegin \DIFdel{Moreover, the maximal contact order at the core is $m>1$}\DIFdelend \DIFaddbegin \end{lemma}

\begin{lemma}\label{lem:stable_vertices} \DIFadd{All the vertices on the circle are stable (as opposed to semistable  vertices introduced by subdivision)}\DIFaddend . \end{lemma}

\DIFdelbegin %DIFDELCMD < \begin{proof}
%DIFDELCMD < %%%
\DIFdel{Stability }\DIFdelend %DIF > \begin{lemma}\label{lem:corecontact}
%DIF >  Among all the flags supported at the core, at least one must have expansion factor strictly greater than $1$.
%DIF > \end{lemma}
\DIFaddbegin 


\begin{proof}[Proof of Lemma \ref{lemma type C0 combinatorial types}]
\DIFadd{Stability of maps to an expansion }\DIFaddend guarantees that at least one stable vertex lies over each vertex of the target. If there is more than one stable vertex, \DIFaddbegin \DIFadd{then we can show }\DIFaddend as in the proof of Lemma \ref{lem:combs} \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend the corresponding locus $\Fcal$ in the moduli space of unexpanded maps has high codimension\DIFaddbegin \DIFadd{, i.e. the collapsing map has fibres of positive dimension}\DIFaddend . As both insertions and the factorisation property for the collapsed map to $\mathbb P^N$ are pulled back from the moduli space of maps without expansions, the contribution vanishes by the projection formula.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{For each }\DIFdelend \DIFaddbegin \end{proof}
\begin{proof}[Proof of Lemma \ref{lem:stable_vertices}]
\DIFadd{Since we are looking at a one-parameter family of tropical maps, for every }\DIFaddend vertex of the target there must \DIFdelbegin \DIFdel{always }\DIFdelend be at least one vertex \DIFdelbegin \DIFdel{in the preimage }\DIFdelend \DIFaddbegin \DIFadd{above it }\DIFaddend which lies on the circle. \DIFdelbegin \DIFdel{This is implied by the fact that the cone $\tau$ corresponding to $\Dcal$ is a ray. }\DIFdelend If the vertex on the circle is strictly semistable -- so that in particular the collapsed map $f_B$ is constant along the corresponding component -- \DIFdelbegin \DIFdel{it can be seen that }\DIFdelend the factorisation condition \DIFaddbegin \DIFadd{for $f_B$ }\DIFaddend is unaffected by $\Gm$-scaling the coordinate corresponding to that vertex in the moduli space of attaching data \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite[\S 2.2]{SMY2}}%DIFAUXCMD
}\DIFaddend , so the map $\Dcal\to\Ecal$ has generic fibre of positive dimension and the contribution vanishes.
\DIFaddbegin \end{proof}
\begin{comment}
\DIFaddend 

\DIFdelbegin \DIFdel{In particular, notice that vertices lying }\DIFdelend \DIFaddbegin \begin{proof}[Proof of Lemma \ref{lem:corecontact}]
\DIFadd{Consider the vertices that lie above $0\in\mathbb R_{\geq0}$ }\DIFaddend on the circle \DIFdelbegin \DIFdel{and tropically mapped to $0\in\mathbb R_{\geq0}$ all have the same contact order at }\DIFdelend \DIFaddbegin \DIFadd{of radius $\delta$: the edges departing from them go directly (up to bubbling) to }\DIFaddend the core, \DIFdelbegin \DIFdel{because by the previous paragraph we may assume that only strictly semistable vertices are encountered on the paths from these vertices to the core. For the same reason, this contact order }\DIFdelend \DIFaddbegin \DIFadd{since there cannot be stable vertices inside the open disc  when we have only one tropical parameter. In particular, all these edges have the same expansion factor }\DIFaddend $m$ \DIFaddbegin \DIFadd{(see again Figure \ref{fig:off_we_go}), which }\DIFaddend is maximal among those of the edges adjacent to the core. \DIFaddbegin \DIFadd{Indeed, all the edges from the core to the circle have the same length $\delta$, so the ones having maximal contact order are those that reach further from the core, and at least one must reach $0\in\mathbb R_{\geq0}$, for otherwise there would be an extra tropical parameter.
}\DIFaddend 

Finally, in case the maximal contact order is $m=1$, \DIFdelbegin \DIFdel{the image tangent vectors }\DIFdelend \DIFaddbegin \DIFadd{in order for $f_B$ to factor, the image of the tangent vectors at the components lying on the circle }\DIFaddend must be linearly dependent \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{in $T_{\PP^N,x}$, where $x$ denotes the image of the core under $f_B$.}\DIFaddend in which case we can deduce as above that factorisation will be satisfied independently of the choice of alignment.
\end{proof}
\DIFaddbegin 

\DIFaddend For the remainder of this section we use the following notation for the combinatorial type $\Delta$; we let $\sqC_0$ denotes the vertex of dual graph corresponding to the core, $\sqC_1,\ldots,\sqC_r$ the other stable vertices (genus zero and lying on the circle, by the previous lemma) and $q_1,\ldots,q_r$ the corresponding splitting nodes.

\DIFaddbegin \end{comment}
\subsubsection{\DIFadd{Lines}}\label{S:lines}
\DIFadd{We begin with a description of the boundary divisor corresponding to a comb whose teeth all have degree $1$. This is at the same time more concrete and peculiar, since the teeth are generically transverse (not just dimensionally) to the hyperplane $H$. For simplicity, we assume that the contact order is $(d)$, concentrated at a marking $x_1$ supported on the elliptic curve $E$ contracted to $H$. The combinatorial type looks as in Figure \ref{fig:comb111}.
}
...
\begin{figure}
 ...
 \tikzset{cross/.style={cross out, draw=black, fill=none, minimum size=2*(#1-\pgflinewidth), inner sep=0pt, outer sep=0pt}, cross/.default={3pt}}
  ...
  \draw[fill=black] \x circle (2pt);
  \foreach \x in {(0,2),(0,1),(0,-1)}
  \draw \x --node[below]{\color{blue} $1$} (4,0);
  \draw (0,0) node{$\ldots$};
  \draw (4,0) --node[below]{\color{blue} $d$} (8,0);
  \draw[->] (0,-2)--(8,-2);
  \draw[fill=white] (4,0) circle (2pt);
  \draw (4,-2) node[cross]{};
  \draw [red] plot [smooth cycle] coordinates {(0,2) (0,1) (0,-1) (2,-1.5) (7,0) (2,2.5)};
 \end{tikzpicture}
 \caption{\DIFaddFL{The combinatorial type of a comb with degree $1$ teeth.}}
 \label{fig:comb111}
\end{figure}


\DIFadd{Start from the moduli space of logarithmic maps. There is a finite splitting morphism:
}\[\DIFadd{\Ecal \to \left(\M{0}{(1)}{\PP^N}{1}^{\times N}\times_{H^N}H\right)\times \Mcal^{\leftrightarrow}_{1,(-1,\ldots,-1,d)},}\]
\DIFadd{where the latter is the moduli space of genus one curves satisfying the }\emph{\DIFadd{double ramification}} \DIFadd{condition. As for $\widetilde{\Dcal}$, the latter should be replaced by $\VZ^{\leftrightarrow}_{1,(-1,\ldots,-1,d)}$, see \S \ref{subsection rubber}.
}

\DIFadd{The left-hand factor can be described very concretely as the choice of a point $p$ of $H$, and $d$ lines through it. A very important observation is that, at least generically, the vertices lying on the circle of radius $\delta$ over $0\in\mathbb R_{\geq 0}$ are automatically aligned. Indeed, if $R_i$ denotes the $i$-th tail of the curve, glued to $E$ at the node $q_i$, then the stable maps provide us with identifications:
}\[\DIFadd{\operatorname{d}\! f_{i,q_i}\colon T_{R_i,q_i}\simeq N_{H/\PP^N,p}}\]
\DIFadd{that can be combined into the $\theta_{i,j}$ of \S \ref{S:ellsing}. So, without further ado, we may produce a contraction $C\to\overline C$ to an elliptic $d$-fold point.
}

\DIFadd{Assume for example that $d\leq N$. Then, for the absolute factorisation to hold, the $d$ lines $f_i(R_i)$ have better to span a $(d-1)$-dimensional subspace of $T_{\PP^N,p}$. The dimension of the moduli space $\widetilde\Dcal$ is then $dN+d-2$. Without loss of generality, we may assume that $H$ is the hyperplane $\{z_0=0\}$, $p$ is the point $[0:\ldots:0:1]$, and that the $d$ lines span the subspace $\{z_{d-1}=\ldots=z_{N-1}=0\}$. The sections $z_i,\ i=d-1,\ldots,N,$ vanish identically near $E$, so they descend automatically to the singularity. The section $z_0$ does so by the construction of $\overline C$. Finally, the sections $z_i,\ i=1,\ldots,d-2,$ impose independent conditions to descend to $\overline C$. All in all, we find that $\Dcal$ has dimension $dN$, making it into a divisor in $\VZ_{1,(d)}(\PP^N|H,d)$.
}

\DIFadd{We can express the factorisation condition tautologically by combining the isomorphisms $\theta_{i,j}$ above, and the differential of $f$. It correponds to the vanishing of the following vector bundle map:
}\[\DIFadd{T_{R_1,q_1}\xrightarrow{(1,\theta_{1,2}\ldots,\theta_{1,d})}\bigoplus_{i=1}^d T_{R_i,q_i}\xrightarrow{\sum\circ\operatorname{d}\!f=\operatorname{d}\!z_1+\ldots\operatorname{d}\!z_{d-2}}N_{H\cap\PP^{d-1}/\PP^{d-1},p}.}\]

\DIFadd{In general, we shall see that the automatic alignment only involves the components corresponding to vertices on the circle that lie over $0$. The internal components on the circle must be aligned by hand. This means that $\widetilde\Dcal$ is a compactified torus bundle over $\Ecal$, see \ref{S:ellsing}. On the other hand, for higher contact order, the vector bundle map $\sum\circ\operatorname{d}\!f$ to $T_{\PP^N,p}$ factors naturally through $T_{H,p}$.
}

\DIFaddend \subsubsection{Recursive description of $\Ecal$} Recall that we have maps
\[
...
Let $\sigma \in \Sigma^{\Kim}$ be the minimal cone containing the image of $\tau$. The dimension of this cone could be large \DIFaddbegin \DIFadd{(see again the left-hand side of Figure \ref{fig:off_we_go}) }\DIFaddend -- its dimension is equal to the \DIFdelbegin \DIFdel{the }\DIFdelend number of levels of the target expansion in the type $\Delta$. The cone $\sigma$ determines a stratum $\Ecal \subseteq \ol\Mcal_{1,\alpha}(\PP^N|H,d)$, and the morphism above restricts to 
\[
\Dcal \to \Ecal.
\] 

\noindent
{\bf Splitting in the ordinary geometry.} The locus $\Ecal$ has the following description. Let $\Lambda_0$ \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{denote }\DIFaddend the collection of stable vertices of the combinatorial type which are mapped to $0\in \RR_{\geq 0}$, and \DIFaddbegin \DIFadd{use }\DIFaddend $\Lambda_{>0}$ for the stable vertices which are mapped to $\RR_{>0}$. Each vertex $v$ has associated discrete data $\Gamma_v$ \DIFaddbegin \DIFadd{(genus, degree, contact order)}\DIFaddend . The vertices over $0$ determine moduli spaces of ordinary relative stable maps, while the higher level vertices determine maps to rubber.  Then $\Ecal$ admits a finite \DIFdelbegin \DIFdel{and surjective }\DIFdelend splitting morphism
...
\noindent
{\bf Exportation to the \DIFdelbegin \DIFdel{ordinary }\DIFdelend \DIFaddbegin \DIFadd{reduced }\DIFaddend geometry.}  Observe that we have a virtual divisor $\widetilde\Dcal \subseteq \widetilde\VZ_{1,\alpha}(\PP^N|H,d)$ such that 
\[
...
\DIFaddbegin \begin{figure}
 ...
 %combinatorial type
  ...
  \draw[fill=black] (0,-1) circle (2 pt);
  \draw (1,1)--node[below,blue]{$m_1$} (2,0) (0,-1)--node[below,blue]{$m_1$} (2,0) (2,0)--node[below,blue]{$m_1+m_2$} (4,0);
  \draw (1,-.5) node[cross]{};
  \draw[fill=white] (2,0) circle (2 pt);

  \draw[->] (2,-1) -- (2,-1.8);

  \draw[fill=black] (0,-2) circle (2 pt);
  \draw[->] (0,-2)--node[below=.5cm]{$\Delta_\tau$} (4,-2);
   \draw (1,-2) node[cross]{};
   \draw (2,-2) node[cross]{};

  \draw [red] plot [smooth cycle] coordinates {(1,1) (0,-1) (3.5,0)};

 %subdivision 
  \draw[->] (6,-2) --node[below=.5cm]{$\sigma$} (10,-2) node[right]{$\ell_1$};
  \draw[->] (6,-2) -- (6,2) node[above]{$\ell_2$};
  \draw[gray] (6,-2) -- (10,1) node[right]{$(m_2,m_1)=\operatorname{Im}(\tau)$};
 \end{tikzpicture}
\caption{\DIFaddFL{From $\Ecal$ to $\widetilde{\Ecal}$. Typical subdivision of a two-dimensional cone $\sigma$ along the image of a ray $\tau$.}}
\label{fig:Etilde}
\end{figure}


\DIFaddend The combinatorics of the flattening can be described explicitly. Consider the map on tropicalizations
\[
\widetilde{\Dcal}^{\operatorname{trop}} \to \Ecal^{\operatorname{trop}}.
\]
The tropical curves on the left inherit a central alignment from $\widetilde{\VZ}_{1,\alpha}(\PP^N|H,d)$, which orders a certain subset of the edge lengths. The polyhedral criterion for equidimensionality is that every cone of the source maps surjectively onto a cone of the target. There is a universal logarithmic modification $\widetilde\Ecal \to \Ecal$ such that $\widetilde\Dcal \to \widetilde\Ecal$ is flat, constructed by taking the minimal subdivision of $\Ecal^{\operatorname{trop}}$ such that cones map surjectively to cones \DIFaddbegin \DIFadd{(see Figure \ref{fig:Etilde})}\DIFaddend .
...
\begin{proof}
Let us assume that the curve does not degenerate, so that the dual graph is as prescribed by $\Delta$. In particular there are \DIFdelbegin \DIFdel{components $\sqC_1,\ldots,\sqC_r$ }\DIFdelend \DIFaddbegin \DIFadd{vertices $\plC_1,\ldots,\plC_r$ }\DIFaddend generically lying on the circle of radius $\delta$. The moduli space of attaching data for the elliptic singularity is the complement of the coordinate hyperplanes in
\DIFdelbegin \begin{displaymath}\DIFdel{\PP\left(\bigoplus_{i=1}^rT_{q_i}C_i\right).}\end{displaymath}
%DIFAUXCMD
\DIFdel{On the boundary, the circle is allowed to miss }\DIFdelend \DIFaddbegin \begin{equation}\DIFadd{\label{eqn:projectivebundle}
 \PP\left(\bigoplus_{i=1}^rT_{q_i}C_i\right);
}\end{equation}

\DIFadd{this can also be thought of as parametrising compatible identifications $\theta_{i,j}\colon T_{C_i,q_i}\simeq T_{C_j,q_j}$, see \S \ref{S:ellsing}. A point in this space determines in particular a map from the rational $r$-fold singularity to the elliptic $r$-fold singularity. That this coincides with the fibre of the aligned moduli space of curves over the ordinary one is well explained in \mbox{%DIFAUXCMD
\cite[\S 3.4]{RSPW}}%DIFAUXCMD
.  Along the boundary, }\DIFaddend some of the \DIFdelbegin \DIFdel{$\sqC_i$ }\DIFdelend \DIFaddbegin \DIFadd{$\plC_i$ }\DIFaddend -- but not all of them! --  \DIFdelbegin \DIFdel{cutting }\DIFdelend \DIFaddbegin \DIFadd{are allowed to move off from the circle, so that the latter cuts }\DIFaddend the edge between them and the core\DIFdelbegin \DIFdel{(correspondingly}\DIFdelend \DIFaddbegin \DIFadd{. Correspondingly}\DIFaddend , there is a destabilisation of the curve \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{(semistable components are represented by crosses in our figures), and we obtain }\DIFaddend an elliptic singularity containing strictly semistable branches \DIFdelbegin \DIFdel{). }\DIFdelend \DIFaddbegin \DIFadd{(}\emph{\DIFadd{sprouting}}\DIFadd{). This was already identified by Smyth as a compactification of the moduli space of attaching data carrying a Gorenstein universal curve \mbox{%DIFAUXCMD
\cite[\S 2.2]{SMY2}}%DIFAUXCMD
.
}

\DIFaddend Tropically, pushing some of the \DIFdelbegin \DIFdel{vertices }\DIFdelend \DIFaddbegin \DIFadd{$\plC_i$ }\DIFaddend off the circle creates new finite-length edges with coordinates $\ell_i\in\RR_{\geq_0}$ (see \DIFaddbegin \DIFadd{the right-hand side of }\DIFaddend Figure \ref{fig:off_we_go}). Since not all of them can be strictly positive, \DIFdelbegin \DIFdel{this represents the fan }\DIFdelend \DIFaddbegin \DIFadd{the resulting fan is that }\DIFaddend of a projective space. \DIFaddbegin \DIFadd{This gives us a fibrewise description of $\widetilde\Dcal\to\Ecal$ over the generic point.
}\DIFaddend 

A slight modification of the argument above is required to account for the fact that all vertices that lie on the circle and are mapped to $0\in\mathbb R_{\geq0}$ by the tropical map are generically aligned on $\Ecal$\DIFdelbegin \DIFdel{: indeed, by Lemma \ref{lemma type C0 combinatorial types}, they all }\DIFdelend \DIFaddbegin \DIFadd{. Indeed, by the combinatorial description of rays in \S \ref{S:combinatorialdescription}, there are no stable vertices inside the circle, so all the edges going from a vertex of $\Lambda_0$ to the core }\DIFaddend have the same \DIFdelbegin \DIFdel{contact order }\DIFdelend \DIFaddbegin \DIFadd{expansion factor }\DIFaddend $m$ \DIFdelbegin \DIFdel{, therefore their }\DIFdelend \DIFaddbegin \DIFadd{(maximal among the contact orders at the core). Therefore, for all $i$ such that $\plC_i\in\Lambda_0$, the }\DIFaddend $T_{q_i}C_i^{\otimes m}$ can be identified \DIFdelbegin \DIFdel{via }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend the target tangent line on the fibre product \eqref{IIIa fibre product}, and, after saturation, the $T_{q_i}C_i$ themselves are canonically isomorphic on $\Ecal$. Thus, only one summand should be taken to represent all of them in the projective bundle \DIFdelbegin \DIFdel{above}\DIFdelend \DIFaddbegin \eqref{eqn:projectivebundle}\DIFaddend .
\end{proof}

\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=.3]{off_we_go}
%DIFDELCMD < %%%
\DIFdelendFL %DIF > \includegraphics[scale=.3]{off_we_go}
\DIFaddbeginFL \begin{tikzpicture}[ipe import]
  ...
  \node[ipe node]
     ...
     %thickenings of l1 and l2
  \draw[line width=.5mm] (317,743.5)--(310,751.5);
  \draw[line width=.5mm] (387,744)--(400,767);

  \pic[fill=white]
     at (353.118, 688.0005) {ipe fdisk};
\end{tikzpicture}
\DIFaddendFL \caption{Moving vertices off the circle creates new tropical parameters.}\label{fig:off_we_go}
\end{figure}

\subsubsection{$\Dcal$ from $\widetilde\Dcal$: first way}\label{subsection D from Dtilde} On $\widetilde\Dcal$ there is a tautological line bundle arising from the modular description of $\widetilde\VZ_{1,\alpha}(\PP^N|H,d)$, namely $\OO(\delta)$. \DIFaddbegin \DIFadd{Recall that the piecewise linear function $\delta$ may be viewed as a global section of the characteristic sheaf of the logarithmic structure on $\widetilde\VZ_{1,\alpha}(\PP^N|H,d)$. The toric geometry dictionary gives rise to an associated Cartier divisor. Alternatively}\sidenote{worth moving to the preliminary section?}\DIFadd{, for any logarithmic stack $X$, the natural coboundary map induced by }$$\DIFadd{0\to \mathcal O_X^\star\to M_X^\text{gp}\to \overline M_X^\text{gp}\to 0}$$ \DIFadd{associates a $\mathcal O_X^\star$-torsor to a global section of the characteristic sheaf. 
}\DIFaddend 


\begin{lemma}\label{lemma fibres of Odelta}
The fibre of $\OO(\delta)$ over a moduli point is naturally isomorphic to
%\begin{equation*} T_{p}C^\prime \otimes \DIFaddbegin \DIFadd{\mathbb }\DIFaddend T \DIFdelbegin \DIFdel{C_0}\DIFdelend \end{equation*}
where $C^\prime$ is any component of $C$ which lies on the circle \DIFaddbegin \DIFadd{of radius $\delta$}\DIFaddend , $p$ is the \DIFdelbegin \DIFdel{adjacent node which points towards }\DIFdelend \DIFaddbegin \DIFadd{node separating it from }\DIFaddend the core, and \DIFdelbegin \DIFdel{$T C_0$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbb T$ }\DIFaddend is the universal tangent line bundle on the moduli space for $\sqC_0$.
\end{lemma}

\DIFaddbegin \DIFadd{The moduli space of radially aligned curves of genus one $\VZ_{1,n}^\text{rad}$ coincides with the Vakil-Zinger blow-up of  $\overline \Mcal_{1,n}$ \mbox{%DIFAUXCMD
\cite[\S 2.3]{VZ}}%DIFAUXCMD
. In particular, up to twisting by exceptional divisors, the tangent line bundles at two marking $\widetilde{\mathbb T}_i$ and $\widetilde{\mathbb T}_j$ are identified (via the dual Hodge bundle $\mathbb E^\vee$), extending the identification on the smooth locus provided by regarding the elliptic curve as an algebraic group. The pullback of this to $\VZ^\leftrightarrow_{\Gamma_{\sqsubset_0}}$ is what we mean by  the universal tangent line bundle $\mathbb T$.
}

\DIFaddend \begin{proof}
Choose a component $C^\prime$ of the curve which lies on the circle. Then $\OO(\delta)$ is equal to the tensor product of tangent line bundles along the path of edges connecting $C^\prime$ to the core. However, this tensor product of line bundles is telescoping: any vertex which appears in the middle of the path will have an incoming and outgoing node \DIFaddbegin \DIFadd{(and possibly other nodes that are irrelevant for this product)}\DIFaddend , and since the vertex corresponds to a smooth rational curve these tangent spaces are naturally dual to each other~\cite[\S 2.2]{VZ}. Thus these factors cancel, and the result follows.
...
\end{proof}
The content of Lemma \DIFdelbegin \DIFdel{\ref{lemma fibres of Odelta} }\DIFdelend \DIFaddbegin \DIFadd{\ref{lem:generic_proj_bundle} }\DIFaddend is that the choice of \DIFaddbegin \DIFadd{a }\DIFaddend point in the fibre of \DIFdelbegin \DIFdel{$\widetilde\Dcal \to \widetilde\Ecal$ }\DIFdelend \DIFaddbegin \DIFadd{$\widetilde\Dcal \to \Ecal$ }\DIFaddend provides a natural identification between the inward-pointing tangent \DIFdelbegin \DIFdel{spaces }\DIFdelend \DIFaddbegin \DIFadd{lines }\DIFaddend on the circle. \DIFdelbegin \DIFdel{This identification amounts to a specific linear dependence relation between the tangent vectors, which gives a point in the moduli space of attaching data for the corresponding elliptic singularity \mbox{%DIFAUXCMD
\cite[\S 2.2]{SMY2}}%DIFAUXCMD
. We may }\DIFdelend \DIFaddbegin \DIFadd{According to Lemma \ref{lemma fibres of Odelta}, they can further be identified with the line bundle $\OO(\delta)\otimes \mathbb T^\vee$ on $\widetilde\Dcal$. %DIF > This identification amounts to a specific linear dependence relation between the tangent vectors, which gives a point in the moduli space of attaching data for the corresponding elliptic singularity \cite[\S 2.2]{SMY2}.
After Corollary \ref{cor:maps_from_elliptic_sing}, we can }\DIFaddend therefore express the factorisation condition in terms of \DIFaddbegin \DIFadd{degeneracy of }\DIFaddend the following natural morphism of vector bundles on $\widetilde\Dcal$:
\begin{equation*} \Sigma \operatorname{d}\!f \colon \OO(\delta)\otimes \DIFdelbegin \DIFdel{TC_0}\DIFdelend \DIFaddbegin \DIFadd{\mathbb T}\DIFaddend ^\vee \to \ev_q^\st T H. \end{equation*}
Here $\Sigma \operatorname{d}\!f$ is defined by summing the images of the tangent vectors at the inward-pointing nodes on the circle, using the natural identification of each of these tangent spaces with \DIFdelbegin \DIFdel{$\OO(\delta)\otimes TC_0^\vee$ }\DIFdelend \DIFaddbegin \DIFadd{$\OO(\delta)\otimes \mathbb T^\vee$ }\DIFaddend established above. \DIFdelbegin \DIFdel{By Lemma \ref{lemma type C0 combinatorial types} we have that it }\DIFdelend \DIFaddbegin \DIFadd{Apart from the case dealt with in \S \ref{S:lines}, the composite:
}\[\DIFadd{\operatorname{pr}\circ\Sigma \operatorname{d}\!f \colon \OO(\delta)\otimes \mathbb T^\vee \to \ev_q^\st T \PP^N\to \ev_q^* N_{H/\PP^N}}\]
 \DIFadd{vanishes, because the contact order of the exterior components with $H$ is strictly higher than $1$. So, $\Sigma \operatorname{d}\!f$ }\DIFaddend takes values in $TH$, rather than $T\PP^N$. 
 \DIFaddbegin 

 \DIFaddend This gives a section of the vector bundle
\begin{equation*} V = (\ev_q^\st TH) \otimes \DIFdelbegin \DIFdel{(TC_0) }\DIFdelend \DIFaddbegin \DIFadd{\mathbb T }\DIFaddend \otimes \OO(-\delta) \end{equation*}
which is transverse and whose vanishing locus coincides with $\Dcal$. \DIFaddbegin \DIFadd{Transversality follows from the construction, because any stratum of the boundary on which $\Sigma \operatorname{d}\!f$ may have vanished has been (blown up and) included into $\mathbb T \otimes \OO(-\delta)$ as a twist. In other words, the circle of radius $\delta$ always passes through at least one component which is not contracted by the map to the expansion. Now, if the vanishing locus of $\Sigma \operatorname{d}\!f$ does not contain any stratum of the boundary, then it is the closure of its intersection with the open stratum, which can be seen to be transverse. }\DIFaddend Thus we obtain:
...
\begin{example}
Consider the locus $\Ecal$ given by $\sigma$ below, and the \DIFdelbegin \DIFdel{exceptional divisor $\widetilde\Dcal_\rho \subseteq \widetilde\Dcal$ given by }\DIFdelend \DIFaddbegin \DIFadd{codimension one locus $\widetilde\Dcal_\rho \subseteq \widetilde\Dcal_\sigma$ given by the two-dimensional (due to the alignment) cone }\DIFaddend $\rho$:
\begin{figure}[!htbp]
\begin{minipage}{0.3\textwidth}
\centering
\begin{tikzpicture}
...
\subsubsection{$\Dcal$ from $\widetilde\Dcal$: second way} \DIFdelbegin \DIFdel{To establish }\DIFdelend %DIF > To establish a more direct connection with the geometry of the fibre product \eqref{IIIa fibre product}, recall that $\widetilde\Ecal$ is built as an explicit modification of a finite cover of \eqref{IIIa fibre product}. For $i \in\{1,\ldots,r\}$ (with the convention established at the end of Lemma \ref{lem:generic_proj_bundle}), it makes sense on $\widetilde\Ecal$ to consider the line bundle $\OO(\delta_i)$ associated to a shortest path from the core to the circle in the $\sqC_i$-direction. 
\DIFaddbegin \DIFadd{We analyse the degeneracy locus of $\Sigma\operatorname{d}\!f$, in order to have }\DIFaddend a more direct \DIFdelbegin \DIFdel{connection with the geometry of }\DIFdelend \DIFaddbegin \DIFadd{construction of $\Dcal$ from }\DIFaddend the fibre product \eqref{IIIa fibre product}\DIFdelbegin \DIFdel{, recall that $\widetilde\Ecal$ is built as an explicit modification of a finite cover of }%DIFDELCMD < \eqref{IIIa fibre product}%%%
\DIFdel{. For $i \in\{1,\ldots,r\}$ (with the convention established at the end of Lemma \ref{lem:generic_proj_bundle}), it makes sense on $\widetilde\Ecal$ to consider the }\DIFdelend \DIFaddbegin \DIFadd{. On $\Ecal$ consider the piecewise linear function $\delta_i$ ($i=1,\ldots,r$), defined as the minimal distance from a component of (i.e. generising to) the core $\plC_0$ to a non-contracted component of the $i$-th tail $\plC_i$, and the associated }\DIFaddend line bundle $\OO(\delta_i)$\DIFdelbegin \DIFdel{associated to a shortest path }\DIFdelend \DIFaddbegin \DIFadd{.
Construct
}\begin{equation*}\DIFadd{\mathcal P=\PP_{\Ecal}\left(\bigoplus_{i=1}^r\OO(\delta_i)\right)\xrightarrow{p}\Ecal.}\end{equation*}
\DIFadd{When all fibre coordinates are non-zero, a point of $\mathcal P$ induces compatible isomorphisms $\OO(\delta_i)\simeq\OO(\delta_j)$ for all $i,j$. We think of this as saying that all the $\plC_i$ are at the same distance }\DIFaddend from the core\DIFdelbegin \DIFdel{to the circle in the $\sqC_i$-direction. Construct
}\begin{displaymath}\DIFdel{\mathcal P=\PP_{\widetilde\Ecal}\left(\bigoplus_{i=1}^r\OO(\delta_i)\right).}\end{displaymath}
%DIFAUXCMD
\DIFdel{Since $\sqC_1,\ldots,\sqC_r$ are always ordered on $\widetilde \Dcal$ (they may lie on the circle, be the first component off the circle, or degenerate and lie in the strict interior),
}\DIFdelend \DIFaddbegin \DIFadd{. When some of the coordinates are $0$, the corresponding tail is further from the core. This corresponds to an alignment of $\plC_1,\ldots,\plC_r$ with respect to the radius $\delta$, that is the distance of the closest of them from the core. Therefore, by the universal property,
%DIF > Since $\sqC_1,\ldots,\sqC_r$ are always ordered on $\widetilde \Dcal$ (they may lie on the circle, be the first component off the circle, or degenerate and lie in the strict interior),
}\DIFaddend there is a morphism $\widetilde \Dcal\to\mathcal P$ lifting \DIFdelbegin \DIFdel{$\widetilde \Dcal\to\widetilde\Ecal$. 
Consider }\DIFdelend \DIFaddbegin \DIFadd{$\widetilde \Dcal\to \Ecal$. 
}

\DIFadd{In order to express factorisation, consider }\DIFaddend the map of vector bundles on $\mathcal P$:
...
The composite, regarded as a section $s$ of $W=\OO_{\mathcal P}(1)\otimes \operatorname{ev}_q^*(TH)$, vanishes precisely where $f$ factors through the elliptic singularity determined by the point in the fibre of $p$. Generically on $\widetilde\Ecal$, this is exactly what we are after (compare with Lemma \ref{lem:generic_proj_bundle}). On the boundary, though, it may happen that a simultaneous degeneration in the base and the fibre makes this factorisation trivial, in the sense that the map $\bar f$ is constant on all the branches of the elliptic singularity. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \begin{lemma}
%DIFDELCMD <  %%%
\DIFdel{For every }\DIFdelend \DIFaddbegin \DIFadd{These loci may be identified explicitly: let $\Lambda=(\Lambda_B,\Lambda_F)$ with }\DIFaddend $\Lambda_B\subseteq \{\sqC_1,\ldots,\sqC_r\}$ and $\Lambda_F=\{\sqC_1,\ldots,\sqC_r\}\setminus \Lambda_B$, \DIFdelbegin \DIFdel{$s$ vanishes along the loci where}\DIFdelend \DIFaddbegin \DIFadd{and let $\Xi_\Lambda$ be the locus in $\mathcal P$ where:
} \begin{itemize} 
 \item[(B)] \DIFadd{$\forall i\in\Lambda_B$, the $i$-th component of the fibre product }\eqref{IIIa fibre product}\DIFadd{, namely the map $f_i\colon C_i\to(\PP^N|H)$, degenerates so that it is constant near the attaching point to the core.
 }\item[(F)] \DIFadd{$\forall j\in\Lambda_F$, }\DIFaddend the \DIFdelbegin \DIFdel{factors corresponding to $\Lambda_B$ in $\widetilde \Ecal$ degenerate so that $f_i\colon C_i\to\PP^N$ is constant in a neighbourhood of $q_i$, and the fibre coordinates $(\lambda_j)_{j\in\Lambda_F}$ along $p\colon\mathcal P\to\widetilde \Ecal$ are set to be }\DIFdelend \DIFaddbegin \DIFadd{$j$-th coordinate in the fibre of $p\colon\mathcal P\to\Ecal$ vanishes.
} \end{itemize} 
\DIFadd{It should be noted that the degeneracy loci of (B) are themselves Gathmann's comb loci \mbox{%DIFAUXCMD
\cite{Ga}}%DIFAUXCMD
, similar to the loci $\pazocal D$ that we are considering, only this time in genus }\DIFaddend $0$\DIFdelbegin \DIFdel{.
}%DIFDELCMD < \end{lemma}
%DIFDELCMD < %%%
\DIFdel{Call these loci }\DIFdelend \DIFaddbegin \DIFadd{, and therefore recursively computable.
}\begin{lemma} 
 \DIFadd{The }\DIFaddend $\Xi_\Lambda$ \DIFdelbegin \DIFdel{for $\Lambda=(\Lambda_B,\Lambda_F)$ (compare }\DIFdelend \DIFaddbegin \DIFadd{are precisely the boundary loci on which $s$ vanishes.
}\end{lemma}

\DIFadd{Compare }\DIFaddend with \cite[\S 3.2]{VZ} for an analogous phenomenon in the non-relative case\DIFdelbegin \DIFdel{); clearly, they }\DIFdelend \DIFaddbegin \DIFadd{. Clearly, these loci }\DIFaddend may be in excess with respect to the expected codimension of $V(s)$, that is $N-1$. These loci are logarithmic substrata; therefore, there exists a logarithmic \DIFdelbegin \DIFdel{blow-up }\DIFdelend \DIFaddbegin \DIFadd{blowup }\DIFaddend that principalises them. We claim \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend $\widetilde\Dcal$ is \DIFdelbegin \DIFdel{(possibly a refinement of) such a blow-up}\DIFdelend \DIFaddbegin \DIFadd{one}\DIFaddend . 
\begin{lemma}
...
\begin{proof}We analyse the ideal of $\Xi_\Lambda$: it is generated by
  \begin{enumerate} 
  \item equations on \DIFdelbegin \DIFdel{$\widetilde\Ecal$ }\DIFdelend \DIFaddbegin \DIFadd{$\Ecal$ }\DIFaddend cutting the locus where $C_i$ degenerates so that $f_i$ is constant on a neighbourhood of $q_i$ for $i\in \Lambda_B$: these correspond to the tropical functions $\delta_i-\lambda(q_i)$ where $\lambda$ is the distance function to the core;
  ...
  \end{enumerate} 
By construction, on $\widetilde \Dcal$ it is always possible to tell which of these tropical lengths is the shortest\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{. Figure \ref{fig:principalisation} represents the situation: }\DIFaddend the circle we have drawn on $\Xi_\Lambda$ is degenerate \DIFdelbegin \DIFdel{-- }\DIFdelend \DIFaddbegin \DIFadd{- }\DIFaddend it passes through no \DIFdelbegin \DIFdel{vertex of positive horizontal degree -- }\DIFdelend \DIFaddbegin \DIFadd{non-contracted vertex - }\DIFaddend therefore we need to decide for a strictly larger circle in order to lift to $\widetilde D$\DIFdelbegin \DIFdel{), which corresponds to principalising the ideal (see Figure \ref{fig:principalisation})}\DIFdelend \DIFaddbegin \DIFadd{. This shows that the ideal is principalised on $\widetilde \Dcal$}\DIFaddend .
\end{proof}

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < \includegraphics[scale=.5]{principalisation}
%DIFDELCMD < %%%
...
\begin{tikzpicture}[ipe import]
 ...
 %%%%%%%%%%%%%%%% left
  \draw
    (64, 720)node[left]{$\plC_2$}
     -- (112, 736)node[above]{$\plC_0$}
     -- (96, 752)node[left]{$\plC_1$};
  \draw[->]
    (64, 704)
     -- (128, 704);
  \pic
  ...
  \draw[red, dotted]
   ...
   %%%%%%%%%%%%%%%%% right  
 \draw[red]
    (188.9249, 712.0225) node[right]{$\delta$}
     arc[start angle=-156.5092, end angle=-115.3692, x radius=85.088, y radius=-96.462];

  \draw
    (192, 720) node[left]{$\plC_2$}
     -- (240, 736)node[above]{$\plC_0$}
     -- (224, 752)node[above]{$\plC_1^0$};
  \draw[->]
    (192, 704)
     -- (256, 704);
  ...
  \draw
    (240, 736)
     -- (256, 736);
  \draw
    (208, 743) node[left]{$\plC_1^2$}
     -- (224, 752);
  \draw
    (208, 759) node[left]{$\plC_1^1$}
     -- (224, 752);
  \pic
     at (208, 743) {ipe disk};
  \pic
...
\end{tikzpicture}
%DIF > \includegraphics[scale=.5]{principalisation}
\DIFaddendFL \caption{On the left, a generic point of $\mathcal P$. On the right, a point of $\Xi$. The dotted circle is the one we draw on $\mathcal P$. The continuous circle represents \DIFdelbeginFL \DIFdelFL{one of the possible choices of }\DIFdelendFL $\delta$ enlarging the dotted one (a stratum of $\widetilde\Dcal$).}\label{fig:principalisation}
\end{figure}

Finally, on $\widetilde\Dcal$ \DIFdelbegin \DIFdel{(or, in fact, on an intermediate blow-up $\widetilde{\mathcal P}\to\mathcal P$) }\DIFdelend %DIF > (or, in fact, on an intermediate blowup $\widetilde{\mathcal P}\to\mathcal P$)
...
\DIFdelbegin \DIFdel{Expressing the pushforward of the Chern classes of $\OO_W(1)$ and $\OO(\widetilde\Xi_\Lambda)$ }\DIFdelend \DIFaddbegin \begin{prop}
 \[ [\DIFadd{\Dcal}]\DIFadd{=e(W(-\Sigma_\Lambda \widetilde\Xi_\Lambda))\cap[\widetilde{\mathcal P}]}\]
 \DIFadd{where $\widetilde{\mathcal P}$ denotes the blow-up of $\mathcal P$ in the $\Xi_\Lambda$.
}\end{prop}

\DIFadd{Applying \mbox{%DIFAUXCMD
\cite[Corollary 4.2.2]{FUL}}%DIFAUXCMD
, we get:
}\begin{cor} \DIFadd{In $A_*(\mathcal P)$ the following formula holds:
 }\[ [\DIFadd{\Dcal}]\DIFadd{=\sum_{\Lambda}\sum_i s_i(\Xi_\Lambda,\mathcal P)c_{N-1-i}(\ev_q^*TH\otimes\OO_\mathcal P(1))}\]
\end{cor}

\DIFadd{It is possible to push the formula further down to $\Ecal$, and applying \mbox{%DIFAUXCMD
\cite[\S3.1]{FUL} }%DIFAUXCMD
we obtain an expression for $[\Dcal]$ }\DIFaddend in terms of\DIFdelbegin \DIFdel{tautological classeson the fibre product underlying $\Ecal$, we get:
}\DIFdelend \DIFaddbegin \DIFadd{:
} \begin{itemize} 
 \item \DIFadd{evaluation classes, such as $\ev_q^*TH$;
 }\item \DIFadd{Chern classes of the logarithmic line bundles $\OO(\delta_i)$;
 }\item \DIFadd{Segre classes of Gathmann's comb loci indexed by $\Lambda_B$.
} \end{itemize} 
...
\DIFdelbegin \DIFdel{Max-Planck-Institut f\"ur Mathematik, Bonn }\DIFdelend \DIFaddbegin \DIFadd{Mathematisches Institut, Universit\"at Heidelberg }\DIFaddend \\
\DIFdelbegin %DIFDELCMD < \href{mailto:battistella@mpim-bonn.mpg.de}{battistella@mpim-bonn.mpg.de}%%%
\DIFdelend \DIFaddbegin \href{mailto:lbattistella@mathi.uni-heidelberg.de}{lbattistella@mathi.uni-heidelberg.de}\DIFaddend \\

\noindent Navid Nabijou \\
\DIFdelbegin \DIFdel{School of Mathematics and }\DIFdelend \DIFaddbegin \DIFadd{Department of Pure Mathematics and Mathematical }\DIFaddend Statistics, University of \DIFdelbegin \DIFdel{Glasgow }\DIFdelend \DIFaddbegin \DIFadd{Cambridge }\DIFaddend \\
\DIFdelbegin %DIFDELCMD < \href{mailto:Navid.Nabijou@glasgow.ac.uk}{navid.nabijou@glasgow.ac.uk}%%%
\DIFdelend \DIFaddbegin \href{mailto:nn333@cam.ac.uk}{nn333@cam.ac.uk}\DIFaddend \\
